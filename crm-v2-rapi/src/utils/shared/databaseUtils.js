...
const UUID_NAMED_FIELDS = new Set([
  ...
  'updatedBy',
  'deletedBy',
  // Non-standard UUID fields from schema.prisma
  'user', // Person.user
  'productVariant', // OpportunityProduct.productVariant
]);
...
async function getPaginatedList({
  ...
  return {
    ...
    perPage: perPage || (query?.pageSize ? +query.pageSize : 10),
...
async function buildAdvancedFilterConditions(filters, filterFields, schema) {
  ...
  for (const [field, value] of Object.entries(validatedFilters)) {
    if (!filterFields.includes(field) || value === undefined) continue;
    ...
    if (value === 'null' || value === null) {
      ...
      filterConditions.push({ [camelCaseField]: { in: value } });
      ...
      filterConditions.push({ [camelCaseField]: value });
    }
  }
...
async function getOptimizedPaginatedList({
  ...
  try {
    ...
    const perPage = Math.min(Math.max(+(pageSize ?? page_size ?? 10), 1), 100);
