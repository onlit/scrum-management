 ...
 * It implements a hardened CORS policy with secure-by-default behavior, comprehensive domain validation,
 * and fail-fast configuration validation to ensure enterprise-grade security.
 *
 * Security Features:
 * - Strict domain allowlist validation
 * - Secure-by-default policy (denies requests without Origin header)
...
function isOriginAllowed(origin) {
  ...
  try {
    ...
    if (!onDevEnv && url.protocol !== 'https:') {
      logEvent(`[CORS_REJECTED]: Non-HTTPS origin rejected: ${origin}`);
      return false;
    }

    // Allow HTTP in development for localhost
    if (
      onDevEnv &&
      url.protocol === 'http:' &&
