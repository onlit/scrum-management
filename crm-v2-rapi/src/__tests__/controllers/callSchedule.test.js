...
// Import the createCallSchedule function from the controller to be tested
const {
  createCallSchedule,
  getAllCallSchedules,
  getCallSchedule,
...
// Mock the Prisma client's callSchedule model to prevent actual database operations during testing
...
// Define a test suite for the createCallSchedule functionality
describe('createCallSchedule', () => {
  ...
  // Test case: successfully creating a callSchedule
  it('should successfully create a callSchedule and return status 201 with the created object', async () => {
    // Define mock data for the callSchedule to be created
    const mockCallScheduleData = {
      name: 'Test CallSchedule',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockCallScheduleData, // Body contains the mock callSchedule data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock callSchedule data
    prisma.callSchedule.create.mockResolvedValue(mockCallScheduleData);

    // Execute the createCallSchedule function with the mocked request and response
    await createCallSchedule(req, res, next);

    // Assert that the callSchedule was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockCallScheduleData); // Check if the created callSchedule data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid callSchedule data to simulate a validation failure
    const invalidCallScheduleData = {
    ...
    const req = {
      ...
      body: invalidCallScheduleData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createCallSchedule function and expect it to throw an error due to validation failure
    await expect(createCallSchedule(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callSchedule.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createCallSchedule function and expect it to throw an error due to database operation failure
    await expect(createCallSchedule(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllCallSchedules` function.
describe('getAllCallSchedules', () => {
  ...
  // Defines a test case that verifies the successful retrieval of callSchedules.
  it('successfully retrieves callSchedules', async () => {
    ...
    // when it successfully fetches callSchedules from the database.
      ...
      results: [
        {
          ...
          name: 'CallSchedule One',
          description: 'Description for CallSchedule One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllCallSchedules` function with the mocked request and response objects.
    await getAllCallSchedules(req, res, next);
...
// Describe block defines a test suite for the `getCallSchedule` function.
describe('getCallSchedule', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a callSchedule
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a callSchedule and return status 200 with the callSchedule data', async () => {
    // Mock data representing a callSchedule returned from the database.
    const mockCallScheduleData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test CallSchedule',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first callSchedule that matches criteria.
    // It's resolved with `mockCallScheduleData`.
    prisma.callSchedule.findFirst.mockResolvedValue(mockCallScheduleData);
    ...
    await getCallSchedule(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock callSchedule data.
    expect(res.json).toHaveBeenCalledWith(mockCallScheduleData);
  });

  // Second test case: it checks the behavior when a callSchedule is not found.
  // The expected behavior is to throw an error, indicating the callSchedule is not found.
  it('should return a 404 error if the callSchedule is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the callSchedule.
    prisma.callSchedule.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent callSchedule ID.
    ...
    await expect(getCallSchedule(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callSchedule.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any callSchedule ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateCallSchedule', () => {
  ...
  // This test verifies that the `updateCallSchedule` function can successfully update a callSchedule
  // in the database and return the updated callSchedule data with a 200 HTTP status code.
  it('should successfully update a callSchedule and return status 200 with the updated callSchedule data', async () => {
    // Mock data for the test, including the callSchedule ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = { name: 'Updated CallSchedule', version: '2.0.0' };
    // The expected result after updating the callSchedule.
    const mockUpdatedCallSchedule = { ...mockRequestBody, id: mockParamsId };

    // Mock the Prisma update operation to resolve with the updated callSchedule data.
    prisma.callSchedule.update.mockResolvedValue(mockUpdatedCallSchedule);
    ...
    await updateCallSchedule(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedCallSchedule);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.callSchedule.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateCallSchedule` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.callSchedule.update.mockRejectedValue(databaseError);
    ...
    await expect(updateCallSchedule(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateCallSchedule` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated callSchedule data, including transformed keys.
    prisma.callSchedule.update.mockResolvedValue({
      id,
    ...
    await updateCallSchedule(req, res);
...
// This describes a test suite for the `deleteCallSchedule` function.
describe('deleteCallSchedule', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a callSchedule and return status 200 with the deleted ID', async () => {
    // Mock ID of the callSchedule to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the callSchedule, used for authorization.
    ...
    prisma.callSchedule.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the callSchedule ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteCallSchedule(req, res, next);
    ...
    // containing the ID of the deleted callSchedule.
    ...
    // including the ID of the callSchedule to delete and the creator's ID for authorization.
    expect(prisma.callSchedule.deleteMany).toHaveBeenCalledWith({
