...
// Import the createCallListPipelineStage function from the controller to be tested
const {
  createCallListPipelineStage,
  getAllCallListPipelineStages,
  getCallListPipelineStage,
  updateCallListPipelineStage,
  deleteCallListPipelineStage,
} = require('#controllers/callListPipelineStage.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's callListPipelineStage model to prevent actual database operations during testing
...
// Define a test suite for the createCallListPipelineStage functionality
describe('createCallListPipelineStage', () => {
  ...
  // Test case: successfully creating a callListPipelineStage
  it('should successfully create a callListPipelineStage and return status 201 with the created object', async () => {
    // Define mock data for the callListPipelineStage to be created
    const mockCallListPipelineStageData = {
      name: 'Test CallListPipelineStage',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockCallListPipelineStageData, // Body contains the mock callListPipelineStage data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock callListPipelineStage data
    prisma.callListPipelineStage.create.mockResolvedValue(
      mockCallListPipelineStageData,
    );

    // Execute the createCallListPipelineStage function with the mocked request and response
    await createCallListPipelineStage(req, res, next);

    // Assert that the callListPipelineStage was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockCallListPipelineStageData); // Check if the created callListPipelineStage data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid callListPipelineStage data to simulate a validation failure
    const invalidCallListPipelineStageData = {
    ...
    const req = {
      ...
      body: invalidCallListPipelineStageData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createCallListPipelineStage function and expect it to throw an error due to validation failure
    await expect(createCallListPipelineStage(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callListPipelineStage.create.mockRejectedValue(
    ...
    // Execute the createCallListPipelineStage function and expect it to throw an error due to database operation failure
    await expect(createCallListPipelineStage(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllCallListPipelineStages` function.
describe('getAllCallListPipelineStages', () => {
  ...
  // Defines a test case that verifies the successful retrieval of callListPipelineStages.
  it('successfully retrieves callListPipelineStages', async () => {
    ...
    // when it successfully fetches callListPipelineStages from the database.
      ...
      results: [
        {
          ...
          name: 'CallListPipelineStage One',
          description: 'Description for CallListPipelineStage One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllCallListPipelineStages` function with the mocked request and response objects.
    await getAllCallListPipelineStages(req, res, next);
...
// Describe block defines a test suite for the `getCallListPipelineStage` function.
describe('getCallListPipelineStage', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a callListPipelineStage
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a callListPipelineStage and return status 200 with the callListPipelineStage data', async () => {
    // Mock data representing a callListPipelineStage returned from the database.
    const mockCallListPipelineStageData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test CallListPipelineStage',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first callListPipelineStage that matches criteria.
    // It's resolved with `mockCallListPipelineStageData`.
    prisma.callListPipelineStage.findFirst.mockResolvedValue(
      mockCallListPipelineStageData,
    );
    ...
    await getCallListPipelineStage(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock callListPipelineStage data.
    expect(res.json).toHaveBeenCalledWith(mockCallListPipelineStageData);
  });

  // Second test case: it checks the behavior when a callListPipelineStage is not found.
  // The expected behavior is to throw an error, indicating the callListPipelineStage is not found.
  it('should return a 404 error if the callListPipelineStage is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the callListPipelineStage.
    prisma.callListPipelineStage.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent callListPipelineStage ID.
    ...
    await expect(getCallListPipelineStage(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callListPipelineStage.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any callListPipelineStage ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateCallListPipelineStage', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateCallListPipelineStage` function can successfully update a callListPipelineStage
  // in the database and return the updated callListPipelineStage data with a 200 HTTP status code.
  it('should successfully update a callListPipelineStage and return status 200 with the updated callListPipelineStage data', async () => {
    // Mock data for the test, including the callListPipelineStage ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated CallListPipelineStage',
      version: '2.0.0',
    };
    // The expected result after updating the callListPipelineStage.
    const mockUpdatedCallListPipelineStage = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma update operation to resolve with the updated callListPipelineStage data.
    prisma.callListPipelineStage.update.mockResolvedValue(
      mockUpdatedCallListPipelineStage,
    );
    ...
    await updateCallListPipelineStage(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedCallListPipelineStage);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.callListPipelineStage.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateCallListPipelineStage` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.callListPipelineStage.update.mockRejectedValue(databaseError);
    ...
    await expect(updateCallListPipelineStage(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateCallListPipelineStage` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated callListPipelineStage data, including transformed keys.
    prisma.callListPipelineStage.update.mockResolvedValue({
      id,
    ...
    await updateCallListPipelineStage(req, res);
...
// This describes a test suite for the `deleteCallListPipelineStage` function.
describe('deleteCallListPipelineStage', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a callListPipelineStage and return status 200 with the deleted ID', async () => {
    // Mock ID of the callListPipelineStage to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the callListPipelineStage, used for authorization.
    ...
    prisma.callListPipelineStage.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the callListPipelineStage ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteCallListPipelineStage(req, res, next);
    ...
    // containing the ID of the deleted callListPipelineStage.
    ...
    // including the ID of the callListPipelineStage to delete and the creator's ID for authorization.
    expect(prisma.callListPipelineStage.deleteMany).toHaveBeenCalledWith({
