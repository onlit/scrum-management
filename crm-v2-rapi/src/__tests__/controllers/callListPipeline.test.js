...
// Import the createCallListPipeline function from the controller to be tested
const {
  createCallListPipeline,
  getAllCallListPipelines,
  getCallListPipeline,
  updateCallListPipeline,
  deleteCallListPipeline,
} = require('#controllers/callListPipeline.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's callListPipeline model to prevent actual database operations during testing
...
// Define a test suite for the createCallListPipeline functionality
describe('createCallListPipeline', () => {
  ...
  // Test case: successfully creating a callListPipeline
  it('should successfully create a callListPipeline and return status 201 with the created object', async () => {
    // Define mock data for the callListPipeline to be created
    const mockCallListPipelineData = {
      name: 'Test CallListPipeline',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockCallListPipelineData, // Body contains the mock callListPipeline data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock callListPipeline data
    prisma.callListPipeline.create.mockResolvedValue(mockCallListPipelineData);

    // Execute the createCallListPipeline function with the mocked request and response
    await createCallListPipeline(req, res, next);

    // Assert that the callListPipeline was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockCallListPipelineData); // Check if the created callListPipeline data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid callListPipeline data to simulate a validation failure
    const invalidCallListPipelineData = {
    ...
    const req = {
      ...
      body: invalidCallListPipelineData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createCallListPipeline function and expect it to throw an error due to validation failure
    await expect(createCallListPipeline(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callListPipeline.create.mockRejectedValue(
    ...
    // Execute the createCallListPipeline function and expect it to throw an error due to database operation failure
    await expect(createCallListPipeline(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllCallListPipelines` function.
describe('getAllCallListPipelines', () => {
  ...
  // Defines a test case that verifies the successful retrieval of callListPipelines.
  it('successfully retrieves callListPipelines', async () => {
    ...
    // when it successfully fetches callListPipelines from the database.
      ...
      results: [
        {
          ...
          name: 'CallListPipeline One',
          description: 'Description for CallListPipeline One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllCallListPipelines` function with the mocked request and response objects.
    await getAllCallListPipelines(req, res, next);
...
// Describe block defines a test suite for the `getCallListPipeline` function.
describe('getCallListPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a callListPipeline
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a callListPipeline and return status 200 with the callListPipeline data', async () => {
    // Mock data representing a callListPipeline returned from the database.
    const mockCallListPipelineData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test CallListPipeline',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first callListPipeline that matches criteria.
    // It's resolved with `mockCallListPipelineData`.
    prisma.callListPipeline.findFirst.mockResolvedValue(
      mockCallListPipelineData,
    );
    ...
    await getCallListPipeline(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock callListPipeline data.
    expect(res.json).toHaveBeenCalledWith(mockCallListPipelineData);
  });

  // Second test case: it checks the behavior when a callListPipeline is not found.
  // The expected behavior is to throw an error, indicating the callListPipeline is not found.
  it('should return a 404 error if the callListPipeline is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the callListPipeline.
    prisma.callListPipeline.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent callListPipeline ID.
    ...
    await expect(getCallListPipeline(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callListPipeline.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any callListPipeline ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateCallListPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateCallListPipeline` function can successfully update a callListPipeline
  // in the database and return the updated callListPipeline data with a 200 HTTP status code.
  it('should successfully update a callListPipeline and return status 200 with the updated callListPipeline data', async () => {
    // Mock data for the test, including the callListPipeline ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated CallListPipeline',
      version: '2.0.0',
    };
    // The expected result after updating the callListPipeline.
    const mockUpdatedCallListPipeline = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma update operation to resolve with the updated callListPipeline data.
    prisma.callListPipeline.update.mockResolvedValue(
      mockUpdatedCallListPipeline,
    );
    ...
    await updateCallListPipeline(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedCallListPipeline);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.callListPipeline.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateCallListPipeline` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.callListPipeline.update.mockRejectedValue(databaseError);
    ...
    await expect(updateCallListPipeline(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateCallListPipeline` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated callListPipeline data, including transformed keys.
    prisma.callListPipeline.update.mockResolvedValue({
      id,
    ...
    await updateCallListPipeline(req, res);
...
// This describes a test suite for the `deleteCallListPipeline` function.
describe('deleteCallListPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a callListPipeline and return status 200 with the deleted ID', async () => {
    // Mock ID of the callListPipeline to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the callListPipeline, used for authorization.
    ...
    prisma.callListPipeline.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the callListPipeline ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteCallListPipeline(req, res, next);
    ...
    // containing the ID of the deleted callListPipeline.
    ...
    // including the ID of the callListPipeline to delete and the creator's ID for authorization.
    expect(prisma.callListPipeline.deleteMany).toHaveBeenCalledWith({
