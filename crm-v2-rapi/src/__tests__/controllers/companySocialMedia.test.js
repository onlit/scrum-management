...
// Import the createCompanySocialMedia function from the controller to be tested
const {
  createCompanySocialMedia,
  getAllCompanySocialMedias,
  getCompanySocialMedia,
  updateCompanySocialMedia,
  deleteCompanySocialMedia,
} = require('#controllers/companySocialMedia.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's companySocialMedia model to prevent actual database operations during testing
...
// Define a test suite for the createCompanySocialMedia functionality
describe('createCompanySocialMedia', () => {
  ...
  // Test case: successfully creating a companySocialMedia
  it('should successfully create a companySocialMedia and return status 201 with the created object', async () => {
    // Define mock data for the companySocialMedia to be created
    const mockCompanySocialMediaData = {
      name: 'Test CompanySocialMedia',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockCompanySocialMediaData, // Body contains the mock companySocialMedia data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock companySocialMedia data
    prisma.companySocialMedia.create.mockResolvedValue(
      mockCompanySocialMediaData,
    );

    // Execute the createCompanySocialMedia function with the mocked request and response
    await createCompanySocialMedia(req, res, next);

    // Assert that the companySocialMedia was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockCompanySocialMediaData); // Check if the created companySocialMedia data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid companySocialMedia data to simulate a validation failure
    const invalidCompanySocialMediaData = {
    ...
    const req = {
      ...
      body: invalidCompanySocialMediaData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createCompanySocialMedia function and expect it to throw an error due to validation failure
    await expect(createCompanySocialMedia(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.companySocialMedia.create.mockRejectedValue(
    ...
    // Execute the createCompanySocialMedia function and expect it to throw an error due to database operation failure
    await expect(createCompanySocialMedia(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllCompanySocialMedias` function.
describe('getAllCompanySocialMedias', () => {
  ...
  // Defines a test case that verifies the successful retrieval of companySocialMedias.
  it('successfully retrieves companySocialMedias', async () => {
    ...
    // when it successfully fetches companySocialMedias from the database.
      ...
      results: [
        {
          ...
          name: 'CompanySocialMedia One',
          description: 'Description for CompanySocialMedia One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllCompanySocialMedias` function with the mocked request and response objects.
    await getAllCompanySocialMedias(req, res, next);
...
// Describe block defines a test suite for the `getCompanySocialMedia` function.
describe('getCompanySocialMedia', () => {
  ...
  // First test case: it checks if the function successfully retrieves a companySocialMedia
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a companySocialMedia and return status 200 with the companySocialMedia data', async () => {
    // Mock data representing a companySocialMedia returned from the database.
    const mockCompanySocialMediaData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test CompanySocialMedia',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first companySocialMedia that matches criteria.
    // It's resolved with `mockCompanySocialMediaData`.
    prisma.companySocialMedia.findFirst.mockResolvedValue(
      mockCompanySocialMediaData,
    );
    ...
    await getCompanySocialMedia(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock companySocialMedia data.
    expect(res.json).toHaveBeenCalledWith(mockCompanySocialMediaData);
  });

  // Second test case: it checks the behavior when a companySocialMedia is not found.
  // The expected behavior is to throw an error, indicating the companySocialMedia is not found.
  it('should return a 404 error if the companySocialMedia is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the companySocialMedia.
    prisma.companySocialMedia.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent companySocialMedia ID.
    ...
    await expect(getCompanySocialMedia(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.companySocialMedia.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any companySocialMedia ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateCompanySocialMedia', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateCompanySocialMedia` function can successfully update a companySocialMedia
  // in the database and return the updated companySocialMedia data with a 200 HTTP status code.
  it('should successfully update a companySocialMedia and return status 200 with the updated companySocialMedia data', async () => {
    // Mock data for the test, including the companySocialMedia ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated CompanySocialMedia',
      version: '2.0.0',
    };
    // The expected result after updating the companySocialMedia.
    const mockUpdatedCompanySocialMedia = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma update operation to resolve with the updated companySocialMedia data.
    prisma.companySocialMedia.update.mockResolvedValue(
      mockUpdatedCompanySocialMedia,
    );
    ...
    await updateCompanySocialMedia(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedCompanySocialMedia);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.companySocialMedia.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateCompanySocialMedia` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.companySocialMedia.update.mockRejectedValue(databaseError);
    ...
    await expect(updateCompanySocialMedia(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateCompanySocialMedia` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated companySocialMedia data, including transformed keys.
    prisma.companySocialMedia.update.mockResolvedValue({
      id,
    ...
    await updateCompanySocialMedia(req, res);
...
// This describes a test suite for the `deleteCompanySocialMedia` function.
describe('deleteCompanySocialMedia', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a companySocialMedia and return status 200 with the deleted ID', async () => {
    // Mock ID of the companySocialMedia to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the companySocialMedia, used for authorization.
    ...
    prisma.companySocialMedia.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the companySocialMedia ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteCompanySocialMedia(req, res, next);
    ...
    // containing the ID of the deleted companySocialMedia.
    ...
    // including the ID of the companySocialMedia to delete and the creator's ID for authorization.
    expect(prisma.companySocialMedia.deleteMany).toHaveBeenCalledWith({
