...
// Import the createSocialMediaType function from the controller to be tested
const {
...
} = require('#controllers/socialMediaType.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's socialMediaType model to prevent actual database operations during testing
...
// Define a test suite for the createSocialMediaType functionality
describe('createSocialMediaType', () => {
  ...
  // Test case: successfully creating a socialMediaType
  it('should successfully create a socialMediaType and return status 201 with the created object', async () => {
    // Define mock data for the socialMediaType to be created
    const mockSocialMediaTypeData = {
      name: 'Test SocialMediaType',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockSocialMediaTypeData, // Body contains the mock socialMediaType data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock socialMediaType data
    prisma.socialMediaType.create.mockResolvedValue(mockSocialMediaTypeData);

    // Execute the createSocialMediaType function with the mocked request and response
    await createSocialMediaType(req, res, next);

    // Assert that the socialMediaType was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockSocialMediaTypeData); // Check if the created socialMediaType data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid socialMediaType data to simulate a validation failure
    const invalidSocialMediaTypeData = {
    ...
    const req = {
      ...
      body: invalidSocialMediaTypeData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createSocialMediaType function and expect it to throw an error due to validation failure
    await expect(createSocialMediaType(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.socialMediaType.create.mockRejectedValue(
    ...
    // Execute the createSocialMediaType function and expect it to throw an error due to database operation failure
    await expect(createSocialMediaType(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllSocialMediaTypes` function.
describe('getAllSocialMediaTypes', () => {
  ...
  // Defines a test case that verifies the successful retrieval of socialMediaTypes.
  it('successfully retrieves socialMediaTypes', async () => {
    ...
    // when it successfully fetches socialMediaTypes from the database.
      ...
      results: [
        {
          ...
          name: 'SocialMediaType One',
          description: 'Description for SocialMediaType One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllSocialMediaTypes` function with the mocked request and response objects.
    await getAllSocialMediaTypes(req, res, next);
...
// Describe block defines a test suite for the `getSocialMediaType` function.
describe('getSocialMediaType', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a socialMediaType
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a socialMediaType and return status 200 with the socialMediaType data', async () => {
    // Mock data representing a socialMediaType returned from the database.
    const mockSocialMediaTypeData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test SocialMediaType',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first socialMediaType that matches criteria.
    // It's resolved with `mockSocialMediaTypeData`.
    prisma.socialMediaType.findFirst.mockResolvedValue(mockSocialMediaTypeData);
    ...
    await getSocialMediaType(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock socialMediaType data.
    expect(res.json).toHaveBeenCalledWith(mockSocialMediaTypeData);
  });

  // Second test case: it checks the behavior when a socialMediaType is not found.
  // The expected behavior is to throw an error, indicating the socialMediaType is not found.
  it('should return a 404 error if the socialMediaType is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the socialMediaType.
    prisma.socialMediaType.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent socialMediaType ID.
    ...
    await expect(getSocialMediaType(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.socialMediaType.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any socialMediaType ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateSocialMediaType', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateSocialMediaType` function can successfully update a socialMediaType
  // in the database and return the updated socialMediaType data with a 200 HTTP status code.
  it('should successfully update a socialMediaType and return status 200 with the updated socialMediaType data', async () => {
    // Mock data for the test, including the socialMediaType ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated SocialMediaType',
      version: '2.0.0',
    };
    // The expected result after updating the socialMediaType.
    const mockUpdatedSocialMediaType = { ...mockRequestBody, id: mockParamsId };

    // Mock the Prisma update operation to resolve with the updated socialMediaType data.
    prisma.socialMediaType.update.mockResolvedValue(mockUpdatedSocialMediaType);
    ...
    await updateSocialMediaType(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedSocialMediaType);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.socialMediaType.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateSocialMediaType` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.socialMediaType.update.mockRejectedValue(databaseError);
    ...
    await expect(updateSocialMediaType(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateSocialMediaType` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated socialMediaType data, including transformed keys.
    prisma.socialMediaType.update.mockResolvedValue({
      id,
    ...
    await updateSocialMediaType(req, res);
...
// This describes a test suite for the `deleteSocialMediaType` function.
describe('deleteSocialMediaType', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a socialMediaType and return status 200 with the deleted ID', async () => {
    // Mock ID of the socialMediaType to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the socialMediaType, used for authorization.
    ...
    prisma.socialMediaType.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the socialMediaType ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteSocialMediaType(req, res, next);
    ...
    // containing the ID of the deleted socialMediaType.
    ...
    // including the ID of the socialMediaType to delete and the creator's ID for authorization.
    expect(prisma.socialMediaType.deleteMany).toHaveBeenCalledWith({
