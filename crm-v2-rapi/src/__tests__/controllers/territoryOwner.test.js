...
// Import the createTerritoryOwner function from the controller to be tested
const {
  createTerritoryOwner,
  getAllTerritoryOwners,
  getTerritoryOwner,
...
// Mock the Prisma client's territoryOwner model to prevent actual database operations during testing
...
// Define a test suite for the createTerritoryOwner functionality
describe('createTerritoryOwner', () => {
  ...
  // Test case: successfully creating a territoryOwner
  it('should successfully create a territoryOwner and return status 201 with the created object', async () => {
    // Define mock data for the territoryOwner to be created
    const mockTerritoryOwnerData = {
      name: 'Test TerritoryOwner',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockTerritoryOwnerData, // Body contains the mock territoryOwner data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock territoryOwner data
    prisma.territoryOwner.create.mockResolvedValue(mockTerritoryOwnerData);

    // Execute the createTerritoryOwner function with the mocked request and response
    await createTerritoryOwner(req, res, next);

    // Assert that the territoryOwner was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockTerritoryOwnerData); // Check if the created territoryOwner data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid territoryOwner data to simulate a validation failure
    const invalidTerritoryOwnerData = {
    ...
    const req = {
      ...
      body: invalidTerritoryOwnerData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createTerritoryOwner function and expect it to throw an error due to validation failure
    await expect(createTerritoryOwner(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.territoryOwner.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createTerritoryOwner function and expect it to throw an error due to database operation failure
    await expect(createTerritoryOwner(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllTerritoryOwners` function.
describe('getAllTerritoryOwners', () => {
  ...
  // Defines a test case that verifies the successful retrieval of territoryOwners.
  it('successfully retrieves territoryOwners', async () => {
    ...
    // when it successfully fetches territoryOwners from the database.
      ...
      results: [
        {
          ...
          name: 'TerritoryOwner One',
          description: 'Description for TerritoryOwner One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllTerritoryOwners` function with the mocked request and response objects.
    await getAllTerritoryOwners(req, res, next);
...
// Describe block defines a test suite for the `getTerritoryOwner` function.
describe('getTerritoryOwner', () => {
  ...
  // First test case: it checks if the function successfully retrieves a territoryOwner
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a territoryOwner and return status 200 with the territoryOwner data', async () => {
    // Mock data representing a territoryOwner returned from the database.
    const mockTerritoryOwnerData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test TerritoryOwner',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first territoryOwner that matches criteria.
    // It's resolved with `mockTerritoryOwnerData`.
    prisma.territoryOwner.findFirst.mockResolvedValue(mockTerritoryOwnerData);
    ...
    await getTerritoryOwner(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock territoryOwner data.
    expect(res.json).toHaveBeenCalledWith(mockTerritoryOwnerData);
  });

  // Second test case: it checks the behavior when a territoryOwner is not found.
  // The expected behavior is to throw an error, indicating the territoryOwner is not found.
  it('should return a 404 error if the territoryOwner is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the territoryOwner.
    prisma.territoryOwner.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent territoryOwner ID.
    ...
    await expect(getTerritoryOwner(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.territoryOwner.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any territoryOwner ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateTerritoryOwner', () => {
  ...
  // This test verifies that the `updateTerritoryOwner` function can successfully update a territoryOwner
  // in the database and return the updated territoryOwner data with a 200 HTTP status code.
  it('should successfully update a territoryOwner and return status 200 with the updated territoryOwner data', async () => {
    // Mock data for the test, including the territoryOwner ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated TerritoryOwner',
      version: '2.0.0',
    };
    // The expected result after updating the territoryOwner.
    const mockUpdatedTerritoryOwner = { ...mockRequestBody, id: mockParamsId };

    // Mock the Prisma update operation to resolve with the updated territoryOwner data.
    prisma.territoryOwner.update.mockResolvedValue(mockUpdatedTerritoryOwner);
    ...
    await updateTerritoryOwner(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedTerritoryOwner);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.territoryOwner.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateTerritoryOwner` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.territoryOwner.update.mockRejectedValue(databaseError);
    ...
    await expect(updateTerritoryOwner(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateTerritoryOwner` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated territoryOwner data, including transformed keys.
    prisma.territoryOwner.update.mockResolvedValue({
      id,
    ...
    await updateTerritoryOwner(req, res);
...
// This describes a test suite for the `deleteTerritoryOwner` function.
describe('deleteTerritoryOwner', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a territoryOwner and return status 200 with the deleted ID', async () => {
    // Mock ID of the territoryOwner to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the territoryOwner, used for authorization.
    ...
    prisma.territoryOwner.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the territoryOwner ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteTerritoryOwner(req, res, next);
    ...
    // containing the ID of the deleted territoryOwner.
    ...
    // including the ID of the territoryOwner to delete and the creator's ID for authorization.
    expect(prisma.territoryOwner.deleteMany).toHaveBeenCalledWith({
