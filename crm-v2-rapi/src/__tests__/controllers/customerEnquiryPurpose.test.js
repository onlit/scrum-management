...
// Import the createCustomerEnquiryPurpose function from the controller to be tested
const {
  createCustomerEnquiryPurpose,
  getAllCustomerEnquiryPurposes,
  getCustomerEnquiryPurpose,
  updateCustomerEnquiryPurpose,
  deleteCustomerEnquiryPurpose,
} = require('#controllers/customerEnquiryPurpose.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's customerEnquiryPurpose model to prevent actual database operations during testing
...
// Define a test suite for the createCustomerEnquiryPurpose functionality
describe('createCustomerEnquiryPurpose', () => {
  ...
  // Test case: successfully creating a customerEnquiryPurpose
  it('should successfully create a customerEnquiryPurpose and return status 201 with the created object', async () => {
    // Define mock data for the customerEnquiryPurpose to be created
    const mockCustomerEnquiryPurposeData = {
      name: 'Test CustomerEnquiryPurpose',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockCustomerEnquiryPurposeData, // Body contains the mock customerEnquiryPurpose data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock customerEnquiryPurpose data
    prisma.customerEnquiryPurpose.create.mockResolvedValue(
      mockCustomerEnquiryPurposeData,
    );

    // Execute the createCustomerEnquiryPurpose function with the mocked request and response
    await createCustomerEnquiryPurpose(req, res, next);

    // Assert that the customerEnquiryPurpose was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockCustomerEnquiryPurposeData); // Check if the created customerEnquiryPurpose data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid customerEnquiryPurpose data to simulate a validation failure
    const invalidCustomerEnquiryPurposeData = {
    ...
    const req = {
      ...
      body: invalidCustomerEnquiryPurposeData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createCustomerEnquiryPurpose function and expect it to throw an error due to validation failure
    await expect(
      createCustomerEnquiryPurpose(req, res, next),
    ).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.customerEnquiryPurpose.create.mockRejectedValue(
    ...
    // Execute the createCustomerEnquiryPurpose function and expect it to throw an error due to database operation failure
    await expect(createCustomerEnquiryPurpose(req, res, next)).rejects.toThrow(
      'Database error', // Assert that the specific error thrown matches the simulated database error
    );
  });
});

// Defines a test suite for the `getAllCustomerEnquiryPurposes` function.
describe('getAllCustomerEnquiryPurposes', () => {
  ...
  // Defines a test case that verifies the successful retrieval of customerEnquiryPurposes.
  it('successfully retrieves customerEnquiryPurposes', async () => {
    ...
    // when it successfully fetches customerEnquiryPurposes from the database.
      ...
      results: [
        {
          ...
          name: 'CustomerEnquiryPurpose One',
          description: 'Description for CustomerEnquiryPurpose One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllCustomerEnquiryPurposes` function with the mocked request and response objects.
    await getAllCustomerEnquiryPurposes(req, res, next);
...
// Describe block defines a test suite for the `getCustomerEnquiryPurpose` function.
describe('getCustomerEnquiryPurpose', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a customerEnquiryPurpose
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a customerEnquiryPurpose and return status 200 with the customerEnquiryPurpose data', async () => {
    // Mock data representing a customerEnquiryPurpose returned from the database.
    const mockCustomerEnquiryPurposeData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test CustomerEnquiryPurpose',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first customerEnquiryPurpose that matches criteria.
    // It's resolved with `mockCustomerEnquiryPurposeData`.
    prisma.customerEnquiryPurpose.findFirst.mockResolvedValue(
      mockCustomerEnquiryPurposeData,
    );
    ...
    await getCustomerEnquiryPurpose(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock customerEnquiryPurpose data.
    expect(res.json).toHaveBeenCalledWith(mockCustomerEnquiryPurposeData);
  });

  // Second test case: it checks the behavior when a customerEnquiryPurpose is not found.
  // The expected behavior is to throw an error, indicating the customerEnquiryPurpose is not found.
  it('should return a 404 error if the customerEnquiryPurpose is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the customerEnquiryPurpose.
    prisma.customerEnquiryPurpose.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent customerEnquiryPurpose ID.
    ...
    await expect(getCustomerEnquiryPurpose(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.customerEnquiryPurpose.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any customerEnquiryPurpose ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateCustomerEnquiryPurpose', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateCustomerEnquiryPurpose` function can successfully update a customerEnquiryPurpose
  // in the database and return the updated customerEnquiryPurpose data with a 200 HTTP status code.
  it('should successfully update a customerEnquiryPurpose and return status 200 with the updated customerEnquiryPurpose data', async () => {
    // Mock data for the test, including the customerEnquiryPurpose ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated CustomerEnquiryPurpose',
      version: '2.0.0',
    };
    // The expected result after updating the customerEnquiryPurpose.
    const mockUpdatedCustomerEnquiryPurpose = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma update operation to resolve with the updated customerEnquiryPurpose data.
    prisma.customerEnquiryPurpose.update.mockResolvedValue(
      mockUpdatedCustomerEnquiryPurpose,
    );
    ...
    await updateCustomerEnquiryPurpose(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedCustomerEnquiryPurpose);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.customerEnquiryPurpose.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateCustomerEnquiryPurpose` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.customerEnquiryPurpose.update.mockRejectedValue(databaseError);
    ...
    await expect(updateCustomerEnquiryPurpose(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateCustomerEnquiryPurpose` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated customerEnquiryPurpose data, including transformed keys.
    prisma.customerEnquiryPurpose.update.mockResolvedValue({
      id,
    ...
    await updateCustomerEnquiryPurpose(req, res);
...
// This describes a test suite for the `deleteCustomerEnquiryPurpose` function.
describe('deleteCustomerEnquiryPurpose', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a customerEnquiryPurpose and return status 200 with the deleted ID', async () => {
    // Mock ID of the customerEnquiryPurpose to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the customerEnquiryPurpose, used for authorization.
    ...
    prisma.customerEnquiryPurpose.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the customerEnquiryPurpose ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteCustomerEnquiryPurpose(req, res, next);
    ...
    // containing the ID of the deleted customerEnquiryPurpose.
    ...
    // including the ID of the customerEnquiryPurpose to delete and the creator's ID for authorization.
    expect(prisma.customerEnquiryPurpose.deleteMany).toHaveBeenCalledWith({
