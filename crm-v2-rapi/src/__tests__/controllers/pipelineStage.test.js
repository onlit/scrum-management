...
describe('createPipelineStage', () => {
  ...
  it('should successfully create a pipelineStage and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(mockPipelineStageData); // Check if the created pipelineStage data was returned in the response
...
// Defines a test suite for the `getAllPipelineStages` function.
describe('getAllPipelineStages', () => {
  ...
  it('successfully retrieves pipelineStages', async () => {
    ...
    // Invoke the `getAllPipelineStages` function with the mocked request and response objects.
    await getAllPipelineStages(req, res, next);
...
describe('getPipelineStage', () => {
  ...
  it('should successfully retrieve a pipelineStage and return status 200 with the pipelineStage data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock pipelineStage data.
    expect(res.json).toHaveBeenCalledWith(mockPipelineStageData);
  });
...
describe('updatePipelineStage', () => {
  ...
  it('should successfully update a pipelineStage and return status 200 with the updated pipelineStage data', async () => {
    ...
    const mockRequestBody = { name: 'Updated PipelineStage', version: '2.0.0' };
    // The expected result after updating the pipelineStage.
    const mockUpdatedPipelineStage = { ...mockRequestBody, id: mockParamsId };

    // Mock the Prisma update operation to resolve with the updated pipelineStage data.
    prisma.pipelineStage.update.mockResolvedValue(mockUpdatedPipelineStage);
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedPipelineStage);
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.pipelineStage.update.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated pipelineStage data, including transformed keys.
...
describe('deletePipelineStage', () => {
  ...
  it('should successfully delete a pipelineStage and return status 200 with the deleted ID', async () => {
    ...
    // Mock ID representing the creator of the pipelineStage, used for authorization.
    ...
    prisma.pipelineStage.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // including the ID of the pipelineStage to delete and the creator's ID for authorization.
    expect(prisma.pipelineStage.deleteMany).toHaveBeenCalledWith({
