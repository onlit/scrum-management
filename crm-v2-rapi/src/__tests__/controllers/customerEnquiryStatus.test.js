...
// Import the createCustomerEnquiryStatus function from the controller to be tested
const {
  createCustomerEnquiryStatus,
  getAllCustomerEnquiryStatuss,
  getCustomerEnquiryStatus,
  updateCustomerEnquiryStatus,
  deleteCustomerEnquiryStatus,
} = require('#controllers/customerEnquiryStatus.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's customerEnquiryStatus model to prevent actual database operations during testing
...
// Define a test suite for the createCustomerEnquiryStatus functionality
describe('createCustomerEnquiryStatus', () => {
  ...
  // Test case: successfully creating a customerEnquiryStatus
  it('should successfully create a customerEnquiryStatus and return status 201 with the created object', async () => {
    // Define mock data for the customerEnquiryStatus to be created
    const mockCustomerEnquiryStatusData = {
      name: 'Test CustomerEnquiryStatus',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockCustomerEnquiryStatusData, // Body contains the mock customerEnquiryStatus data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock customerEnquiryStatus data
    prisma.customerEnquiryStatus.create.mockResolvedValue(
      mockCustomerEnquiryStatusData,
    );

    // Execute the createCustomerEnquiryStatus function with the mocked request and response
    await createCustomerEnquiryStatus(req, res, next);

    // Assert that the customerEnquiryStatus was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockCustomerEnquiryStatusData); // Check if the created customerEnquiryStatus data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid customerEnquiryStatus data to simulate a validation failure
    const invalidCustomerEnquiryStatusData = {
    ...
    const req = {
      ...
      body: invalidCustomerEnquiryStatusData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createCustomerEnquiryStatus function and expect it to throw an error due to validation failure
    await expect(createCustomerEnquiryStatus(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.customerEnquiryStatus.create.mockRejectedValue(
    ...
    // Execute the createCustomerEnquiryStatus function and expect it to throw an error due to database operation failure
    await expect(createCustomerEnquiryStatus(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllCustomerEnquiryStatuss` function.
describe('getAllCustomerEnquiryStatuss', () => {
  ...
  // Defines a test case that verifies the successful retrieval of customerEnquiryStatuss.
  it('successfully retrieves customerEnquiryStatuss', async () => {
    ...
    // when it successfully fetches customerEnquiryStatuss from the database.
      ...
      results: [
        {
          ...
          name: 'CustomerEnquiryStatus One',
          description: 'Description for CustomerEnquiryStatus One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllCustomerEnquiryStatuss` function with the mocked request and response objects.
    await getAllCustomerEnquiryStatuss(req, res, next);
...
// Describe block defines a test suite for the `getCustomerEnquiryStatus` function.
describe('getCustomerEnquiryStatus', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a customerEnquiryStatus
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a customerEnquiryStatus and return status 200 with the customerEnquiryStatus data', async () => {
    // Mock data representing a customerEnquiryStatus returned from the database.
    const mockCustomerEnquiryStatusData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test CustomerEnquiryStatus',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first customerEnquiryStatus that matches criteria.
    // It's resolved with `mockCustomerEnquiryStatusData`.
    prisma.customerEnquiryStatus.findFirst.mockResolvedValue(
      mockCustomerEnquiryStatusData,
    );
    ...
    await getCustomerEnquiryStatus(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock customerEnquiryStatus data.
    expect(res.json).toHaveBeenCalledWith(mockCustomerEnquiryStatusData);
  });

  // Second test case: it checks the behavior when a customerEnquiryStatus is not found.
  // The expected behavior is to throw an error, indicating the customerEnquiryStatus is not found.
  it('should return a 404 error if the customerEnquiryStatus is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the customerEnquiryStatus.
    prisma.customerEnquiryStatus.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent customerEnquiryStatus ID.
    ...
    await expect(getCustomerEnquiryStatus(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.customerEnquiryStatus.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any customerEnquiryStatus ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateCustomerEnquiryStatus', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateCustomerEnquiryStatus` function can successfully update a customerEnquiryStatus
  // in the database and return the updated customerEnquiryStatus data with a 200 HTTP status code.
  it('should successfully update a customerEnquiryStatus and return status 200 with the updated customerEnquiryStatus data', async () => {
    // Mock data for the test, including the customerEnquiryStatus ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated CustomerEnquiryStatus',
      version: '2.0.0',
    };
    // The expected result after updating the customerEnquiryStatus.
    const mockUpdatedCustomerEnquiryStatus = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma update operation to resolve with the updated customerEnquiryStatus data.
    prisma.customerEnquiryStatus.update.mockResolvedValue(
      mockUpdatedCustomerEnquiryStatus,
    );
    ...
    await updateCustomerEnquiryStatus(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedCustomerEnquiryStatus);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.customerEnquiryStatus.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updateCustomerEnquiryStatus` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.customerEnquiryStatus.update.mockRejectedValue(databaseError);
    ...
    await expect(updateCustomerEnquiryStatus(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateCustomerEnquiryStatus` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated customerEnquiryStatus data, including transformed keys.
    prisma.customerEnquiryStatus.update.mockResolvedValue({
      id,
    ...
    await updateCustomerEnquiryStatus(req, res);
...
// This describes a test suite for the `deleteCustomerEnquiryStatus` function.
describe('deleteCustomerEnquiryStatus', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a customerEnquiryStatus and return status 200 with the deleted ID', async () => {
    // Mock ID of the customerEnquiryStatus to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the customerEnquiryStatus, used for authorization.
    ...
    prisma.customerEnquiryStatus.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the customerEnquiryStatus ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteCustomerEnquiryStatus(req, res, next);
    ...
    // containing the ID of the deleted customerEnquiryStatus.
    ...
    // including the ID of the customerEnquiryStatus to delete and the creator's ID for authorization.
    expect(prisma.customerEnquiryStatus.deleteMany).toHaveBeenCalledWith({
