...
// Import the createPersonRelationshipHistory function from the controller to be tested
const {
  createPersonRelationshipHistory,
  getAllPersonRelationshipHistorys,
  getPersonRelationshipHistory,
  updatePersonRelationshipHistory,
  deletePersonRelationshipHistory,
} = require('#controllers/personRelationshipHistory.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's personRelationshipHistory model to prevent actual database operations during testing
...
// Define a test suite for the createPersonRelationshipHistory functionality
describe('createPersonRelationshipHistory', () => {
  ...
  // Test case: successfully creating a personRelationshipHistory
  it('should successfully create a personRelationshipHistory and return status 201 with the created object', async () => {
    // Define mock data for the personRelationshipHistory to be created
    const mockPersonRelationshipHistoryData = {
      name: 'Test PersonRelationshipHistory',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockPersonRelationshipHistoryData, // Body contains the mock personRelationshipHistory data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock personRelationshipHistory data
    prisma.personRelationshipHistory.create.mockResolvedValue(
      mockPersonRelationshipHistoryData,
    );

    // Execute the createPersonRelationshipHistory function with the mocked request and response
    await createPersonRelationshipHistory(req, res, next);

    // Assert that the personRelationshipHistory was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockPersonRelationshipHistoryData); // Check if the created personRelationshipHistory data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid personRelationshipHistory data to simulate a validation failure
    const invalidPersonRelationshipHistoryData = {
    ...
    const req = {
      ...
      body: invalidPersonRelationshipHistoryData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createPersonRelationshipHistory function and expect it to throw an error due to validation failure
    await expect(
      createPersonRelationshipHistory(req, res, next),
    ).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.personRelationshipHistory.create.mockRejectedValue(
    ...
    // Execute the createPersonRelationshipHistory function and expect it to throw an error due to database operation failure
    await expect(
      createPersonRelationshipHistory(req, res, next),
    ).rejects.toThrow(
...
// Defines a test suite for the `getAllPersonRelationshipHistorys` function.
describe('getAllPersonRelationshipHistorys', () => {
  ...
  // Defines a test case that verifies the successful retrieval of personRelationshipHistorys.
  it('successfully retrieves personRelationshipHistorys', async () => {
    ...
    // when it successfully fetches personRelationshipHistorys from the database.
      ...
      results: [
        {
          ...
          name: 'PersonRelationshipHistory One',
          description: 'Description for PersonRelationshipHistory One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllPersonRelationshipHistorys` function with the mocked request and response objects.
    await getAllPersonRelationshipHistorys(req, res, next);
...
// Describe block defines a test suite for the `getPersonRelationshipHistory` function.
describe('getPersonRelationshipHistory', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a personRelationshipHistory
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a personRelationshipHistory and return status 200 with the personRelationshipHistory data', async () => {
    // Mock data representing a personRelationshipHistory returned from the database.
    const mockPersonRelationshipHistoryData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test PersonRelationshipHistory',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first personRelationshipHistory that matches criteria.
    // It's resolved with `mockPersonRelationshipHistoryData`.
    prisma.personRelationshipHistory.findFirst.mockResolvedValue(
      mockPersonRelationshipHistoryData,
    );
    ...
    await getPersonRelationshipHistory(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock personRelationshipHistory data.
    expect(res.json).toHaveBeenCalledWith(mockPersonRelationshipHistoryData);
  });

  // Second test case: it checks the behavior when a personRelationshipHistory is not found.
  // The expected behavior is to throw an error, indicating the personRelationshipHistory is not found.
  it('should return a 404 error if the personRelationshipHistory is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the personRelationshipHistory.
    prisma.personRelationshipHistory.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent personRelationshipHistory ID.
    ...
    await expect(getPersonRelationshipHistory(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.personRelationshipHistory.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any personRelationshipHistory ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updatePersonRelationshipHistory', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updatePersonRelationshipHistory` function can successfully update a personRelationshipHistory
  // in the database and return the updated personRelationshipHistory data with a 200 HTTP status code.
  it('should successfully update a personRelationshipHistory and return status 200 with the updated personRelationshipHistory data', async () => {
    // Mock data for the test, including the personRelationshipHistory ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'Updated PersonRelationshipHistory',
      version: '2.0.0',
    };
    // The expected result after updating the personRelationshipHistory.
    const mockUpdatedPersonRelationshipHistory = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma update operation to resolve with the updated personRelationshipHistory data.
    prisma.personRelationshipHistory.update.mockResolvedValue(
      mockUpdatedPersonRelationshipHistory,
    );
    ...
    await updatePersonRelationshipHistory(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedPersonRelationshipHistory);
    // Verify that the Prisma update method was called with the correct parameters.
    expect(prisma.personRelationshipHistory.update).toHaveBeenCalledWith({
      where: { id: mockParamsId },
      data: mockRequestBody,
    });
  });

  // This test ensures that the `updatePersonRelationshipHistory` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.personRelationshipHistory.update.mockRejectedValue(databaseError);
    ...
    await expect(
      updatePersonRelationshipHistory(req, {}, next),
    ).rejects.toThrow('Database error');
  });

  // This test checks that the `updatePersonRelationshipHistory` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated personRelationshipHistory data, including transformed keys.
    prisma.personRelationshipHistory.update.mockResolvedValue({
      id,
    ...
    await updatePersonRelationshipHistory(req, res);
...
// This describes a test suite for the `deletePersonRelationshipHistory` function.
describe('deletePersonRelationshipHistory', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a personRelationshipHistory and return status 200 with the deleted ID', async () => {
    // Mock ID of the personRelationshipHistory to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the personRelationshipHistory, used for authorization.
    ...
    prisma.personRelationshipHistory.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the personRelationshipHistory ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deletePersonRelationshipHistory(req, res, next);
    ...
    // containing the ID of the deleted personRelationshipHistory.
    ...
    // including the ID of the personRelationshipHistory to delete and the creator's ID for authorization.
    expect(prisma.personRelationshipHistory.deleteMany).toHaveBeenCalledWith({
