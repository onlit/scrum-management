...
// Import the createActionPlan function from the controller to be tested
const {
  createActionPlan,
  getAllActionPlans,
  getActionPlan,
  updateActionPlan,
  deleteActionPlan,
} = require('#controllers/actionPlan.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's actionPlan model to prevent actual database operations during testing
...
// Define a test suite for the createActionPlan functionality
describe('createActionPlan', () => {
  ...
  // Test case: successfully creating a actionPlan
  it('should successfully create a actionPlan and return status 201 with the created object', async () => {
    // Define mock data for the actionPlan to be created
    const mockActionPlanData = {
      name: 'Test ActionPlan',
      version: '1.0.0',
    };
    ...
    const req = {
      ...
      body: mockActionPlanData, // Body contains the mock actionPlan data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock actionPlan data
    prisma.actionPlan.create.mockResolvedValue(mockActionPlanData);

    // Execute the createActionPlan function with the mocked request and response
    await createActionPlan(req, res, next);

    // Assert that the actionPlan was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockActionPlanData); // Check if the created actionPlan data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid actionPlan data to simulate a validation failure
    const invalidActionPlanData = {
    ...
    const req = {
      ...
      body: invalidActionPlanData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createActionPlan function and expect it to throw an error due to validation failure
    await expect(createActionPlan(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.actionPlan.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createActionPlan function and expect it to throw an error due to database operation failure
    await expect(createActionPlan(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllActionPlans` function.
describe('getAllActionPlans', () => {
  ...
  // Defines a test case that verifies the successful retrieval of actionPlans.
  it('successfully retrieves actionPlans', async () => {
    ...
    // when it successfully fetches actionPlans from the database.
      ...
      results: [
        {
          ...
          name: 'ActionPlan One',
          description: 'Description for ActionPlan One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllActionPlans` function with the mocked request and response objects.
    await getAllActionPlans(req, res, next);
...
// Describe block defines a test suite for the `getActionPlan` function.
describe('getActionPlan', () => {
  ...
  // First test case: it checks if the function successfully retrieves a actionPlan
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a actionPlan and return status 200 with the actionPlan data', async () => {
    // Mock data representing a actionPlan returned from the database.
    const mockActionPlanData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test ActionPlan',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first actionPlan that matches criteria.
    // It's resolved with `mockActionPlanData`.
    prisma.actionPlan.findFirst.mockResolvedValue(mockActionPlanData);
    ...
    await getActionPlan(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock actionPlan data.
    expect(res.json).toHaveBeenCalledWith(mockActionPlanData);
  });

  // Second test case: it checks the behavior when a actionPlan is not found.
  // The expected behavior is to throw an error, indicating the actionPlan is not found.
  it('should return a 404 error if the actionPlan is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the actionPlan.
    prisma.actionPlan.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent actionPlan ID.
    ...
    await expect(getActionPlan(req, res, next)).rejects.toThrow(
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.actionPlan.findFirst.mockRejectedValue(new Error('Database error'));
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any actionPlan ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateActionPlan', () => {
  ...
  // This test verifies that the `updateActionPlan` function can successfully update a actionPlan
  // in the database and return the updated actionPlan data with a 200 HTTP status code.
  it('should successfully update a actionPlan and return status 200 with the updated actionPlan data', async () => {
    // Mock data for the test, including the actionPlan ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = { name: 'Updated ActionPlan', version: '2.0.0' };
    // The expected result after updating the actionPlan.
    const mockUpdatedActionPlan = { ...mockRequestBody, id: mockParamsId };

    // Mock the Prisma update operation to resolve with the updated actionPlan data.
    prisma.actionPlan.update.mockResolvedValue(mockUpdatedActionPlan);
    ...
    await updateActionPlan(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedActionPlan);
  ...
  // This test ensures that the `updateActionPlan` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.actionPlan.update.mockRejectedValue(databaseError);
    ...
    await expect(updateActionPlan(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updateActionPlan` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated actionPlan data, including transformed keys.
...
// This describes a test suite for the `deleteActionPlan` function.
describe('deleteActionPlan', () => {
  ...
  it('should successfully delete a actionPlan and return status 200 with the deleted ID', async () => {
    // Mock ID of the actionPlan to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the actionPlan, used for authorization.
    ...
    prisma.actionPlan.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the actionPlan ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deleteActionPlan(req, res, next);
    ...
    // containing the ID of the deleted actionPlan.
    ...
    // including the ID of the actionPlan to delete and the creator's ID for authorization.
    expect(prisma.actionPlan.deleteMany).toHaveBeenCalledWith({
