...
// Import the createPipeline function from the controller to be tested
const {
  createPipeline,
  getAllPipelines,
  getPipeline,
  updatePipeline,
  deletePipeline,
} = require('#controllers/pipeline.controller.js');
const { getPaginatedList } = require('#utils/shared/databaseUtils.js');

// Mock the Prisma client's pipeline model to prevent actual database operations during testing
...
// Define a test suite for the createPipeline functionality
describe('createPipeline', () => {
  ...
  // Test case: successfully creating a pipeline
  it('should successfully create a pipeline and return status 201 with the created object', async () => {
    // Define mock data for the pipeline to be created
    ...
    const req = {
      ...
      body: mockPipelineData, // Body contains the mock pipeline data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock pipeline data
    prisma.pipeline.create.mockResolvedValue(mockPipelineData);

    // Execute the createPipeline function with the mocked request and response
    await createPipeline(req, res, next);

    // Assert that the pipeline was created successfully
    expect(res.status).toHaveBeenCalledWith(201); // Check if response status was set to 201
    expect(res.json).toHaveBeenCalledWith(mockPipelineData); // Check if the created pipeline data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid pipeline data to simulate a validation failure
    ...
    const req = {
      ...
      body: invalidPipelineData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createPipeline function and expect it to throw an error due to validation failure
    await expect(createPipeline(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.pipeline.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createPipeline function and expect it to throw an error due to database operation failure
    await expect(createPipeline(req, res, next)).rejects.toThrow(
...
// Defines a test suite for the `getAllPipelines` function.
describe('getAllPipelines', () => {
  ...
  // Defines a test case that verifies the successful retrieval of pipelines.
  it('successfully retrieves pipelines', async () => {
    ...
    // when it successfully fetches pipelines from the database.
      ...
      results: [
        {
          ...
          name: 'Pipeline One',
          description: 'Description for Pipeline One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllPipelines` function with the mocked request and response objects.
    await getAllPipelines(req, res, next);
...
// Describe block defines a test suite for the `getPipeline` function.
describe('getPipeline', () => {
  ...
  // First test case: it checks if the function successfully retrieves a pipeline
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a pipeline and return status 200 with the pipeline data', async () => {
    // Mock data representing a pipeline returned from the database.
    const mockPipelineData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test Pipeline',
      version: '1.0.0',
    ...
    // Mock the Prisma call to find the first pipeline that matches criteria.
    // It's resolved with `mockPipelineData`.
    prisma.pipeline.findFirst.mockResolvedValue(mockPipelineData);
    ...
    await getPipeline(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body is the mock pipeline data.
    expect(res.json).toHaveBeenCalledWith(mockPipelineData);
  });

  // Second test case: it checks the behavior when a pipeline is not found.
  // The expected behavior is to throw an error, indicating the pipeline is not found.
  it('should return a 404 error if the pipeline is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the pipeline.
    prisma.pipeline.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent pipeline ID.
    ...
    await expect(getPipeline(req, res, next)).rejects.toThrow(
      'Pipeline not found',
    );
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.pipeline.findFirst.mockRejectedValue(new Error('Database error'));
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any pipeline ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
    ...
    await expect(getPipeline(req, res, next)).rejects.toThrow('Database error');
  });
});
...
describe('updatePipeline', () => {
  ...
  // This test verifies that the `updatePipeline` function can successfully update a pipeline
  // in the database and return the updated pipeline data with a 200 HTTP status code.
  it('should successfully update a pipeline and return status 200 with the updated pipeline data', async () => {
    // Mock data for the test, including the pipeline ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = { name: 'Updated Pipeline', version: '2.0.0' };
    // The expected result after updating the pipeline.
    const mockUpdatedPipeline = { ...mockRequestBody, id: mockParamsId };

    // Mock the Prisma update operation to resolve with the updated pipeline data.
    prisma.pipeline.update.mockResolvedValue(mockUpdatedPipeline);
    ...
    await updatePipeline(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(mockUpdatedPipeline);
  ...
  // This test ensures that the `updatePipeline` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma update operation.
    const databaseError = new Error('Database error');
    prisma.pipeline.update.mockRejectedValue(databaseError);
    ...
    await expect(updatePipeline(req, {}, next)).rejects.toThrow(
      'Database error',
    );
  });

  // This test checks that the `updatePipeline` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    // Mock the Prisma update operation to resolve with the updated pipeline data, including transformed keys.
...
// This describes a test suite for the `deletePipeline` function.
describe('deletePipeline', () => {
  ...
  it('should successfully delete a pipeline and return status 200 with the deleted ID', async () => {
    // Mock ID of the pipeline to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // Mock ID representing the creator of the pipeline, used for authorization.
    ...
    prisma.pipeline.deleteMany.mockResolvedValue({ count: 1 });
    ...
    // Mocks the HTTP request object, including the pipeline ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { isAuthenticated: true },
    };
    ...
    await deletePipeline(req, res, next);
    ...
    // containing the ID of the deleted pipeline.
    ...
    // including the ID of the pipeline to delete and the creator's ID for authorization.
