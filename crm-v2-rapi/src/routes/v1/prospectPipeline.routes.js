 ...
 * CRUD operations related to prospectPipeline. It imports middleware functions for authentication,
 * authorization, and error handling, as well as controller functions for handling specific
 * CRUD operations on prospectPipeline.
 *
...
} = require('#controllers/prospectPipeline.controller.js');
...
router.get('/', auth, wrapExpressAsync(getAllProspectPipeline, 'prospect_pipeline_get_all'));

router.get(
  '/bar-chart',
  auth,
  wrapExpressAsync(getProspectPipelineBarChartData, 'prospect_pipeline_bar_chart'),
);

router.get('/:id', auth, wrapExpressAsync(getProspectPipeline, 'prospect_pipeline_get_by_id'));
