...
describe('createPersonRelationship', () => {
  ...
  it('should successfully create a personRelationship and return status 201 with the created object', async () => {
    ...
    const mockPersonRelationshipData = {
      relationshipId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonRelationshipData),
    ); // Check if the created personRelationship data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createPersonRelationship(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllPersonRelationship` function.
describe('getAllPersonRelationship', () => {
  ...
  it('successfully retrieves personRelationships', async () => {
    ...
    // Invoke the `getAllPersonRelationship` function with the mocked request and response objects.
    await getAllPersonRelationship(req, res, next);
...
describe('getPersonRelationship', () => {
  ...
  it('should successfully retrieve a personRelationship and return status 200 with the personRelationship data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock personRelationship data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonRelationshipData),
    );
...
describe('updatePersonRelationship', () => {
  ...
  it('should successfully update a personRelationship and return status 200 with the updated personRelationship data', async () => {
    ...
    prisma.personRelationship.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.personRelationship.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.personRelationship.updateMany.mockRejectedValue(databaseError);
...
describe('deletePersonRelationship', () => {
  ...
  it('should successfully delete a personRelationship and return status 200 with the deleted ID', async () => {
    ...
    prisma.personRelationshipHistory.updateMany.mockResolvedValue({ count: 0 });
