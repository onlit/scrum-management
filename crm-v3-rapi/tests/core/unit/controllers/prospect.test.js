...
// Import the createProspect function from the controller to be tested
const {
  createProspect,
  getAllProspect,
  getProspect,
...
// Mock the Prisma client's prospect model to prevent actual database operations during testing
...
// Define a test suite for the createProspect functionality
describe('createProspect', () => {
  ...
  // Test case: successfully creating a prospect
  it('should successfully create a prospect and return status 201 with the created object', async () => {
    // Define mock data for the prospect to be created
    const mockProspectData = {
      ownerId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      categoryId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    ...
    const req = {
      ...
      body: mockProspectData, // Body contains the mock prospect data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock prospect data
    prisma.prospect.create.mockResolvedValue(mockProspectData);

    // Execute the createProspect function with the mocked request and response
    await createProspect(req, res, next);

    // Assert that the prospect was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockProspectData),
    ); // Check if the created prospect data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid prospect data to simulate a validation failure
    ...
    const req = {
      ...
      body: invalidProspectData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createProspect function and expect it to throw an error due to validation failure
    await expect(createProspect(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.prospect.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createProspect function and expect it to throw an error due to database operation failure
    await expect(createProspect(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllProspect` function.
describe('getAllProspect', () => {
  ...
  // Defines a test case that verifies the successful retrieval of prospects.
  it('successfully retrieves prospects', async () => {
    ...
    // when it successfully fetches prospects from the database.
      ...
      results: [
        {
          ...
          name: 'Prospect One',
          description: 'Description for Prospect One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllProspect` function with the mocked request and response objects.
    await getAllProspect(req, res, next);
...
// Describe block defines a test suite for the `getProspect` function.
describe('getProspect', () => {
  ...
  // First test case: it checks if the function successfully retrieves a prospect
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a prospect and return status 200 with the prospect data', async () => {
    // Mock data representing a prospect returned from the database.
    const mockProspectData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test Prospect',
    };

    // Mock the Prisma call to find the first prospect that matches criteria.
    // It's resolved with `mockProspectData`.
    prisma.prospect.findFirst.mockResolvedValue(mockProspectData);
    ...
    await getProspect(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock prospect data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockProspectData),
    );
  ...
  // Second test case: it checks the behavior when a prospect is not found.
  // The expected behavior is to throw an error, indicating the prospect is not found.
  it('should return a 404 error if the prospect is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the prospect.
    prisma.prospect.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent prospect ID.
    ...
    await expect(getProspect(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.prospect.findFirst.mockRejectedValue(new Error('Database error'));
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any prospect ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateProspect', () => {
  ...
  // This test verifies that the `updateProspect` function can successfully update a prospect
  // in the database and return the updated prospect data with a 200 HTTP status code.
  it('should successfully update a prospect and return status 200 with the updated prospect data', async () => {
    // Mock data for the test, including the prospect ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      qualificationScore: 42,
    };
    // The expected result after updating the prospect.
    ...
    prisma.prospect.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.prospect.findFirst.mockResolvedValue(mockUpdatedProspect);
    ...
    await updateProspect(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    // Verify that the Prisma updateMany method was called.
    expect(prisma.prospect.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updateProspect` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.prospect.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(updateProspect(req, {}, next)).rejects.toThrow();
  });

  // This test checks that the `updateProspect` function transforms the keys of the request body
...
// This describes a test suite for the `deleteProspect` function.
describe('deleteProspect', () => {
  ...
  it('should successfully delete a prospect and return status 200 with the deleted ID', async () => {
    // Mock ID of the prospect to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    prisma.prospectProduct.updateMany.mockResolvedValue({ count: 0 });
    ...
    // Mocks the HTTP request object, including the prospect ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: {
        id: 'user-123',
    ...
    await deleteProspect(req, res, next);
    ...
    // containing the ID of the deleted prospect.
