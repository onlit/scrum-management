...
describe('createOpportunityInfluencer', () => {
  ...
  it('should successfully create a opportunityInfluencer and return status 201 with the created object', async () => {
    ...
    const mockOpportunityInfluencerData = {
      companyContactId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      role: 'd_compute_test_role',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityInfluencerData),
    ); // Check if the created opportunityInfluencer data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createOpportunityInfluencer(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllOpportunityInfluencer` function.
describe('getAllOpportunityInfluencer', () => {
  ...
  it('successfully retrieves opportunityInfluencers', async () => {
    ...
    // Invoke the `getAllOpportunityInfluencer` function with the mocked request and response objects.
    await getAllOpportunityInfluencer(req, res, next);
...
describe('getOpportunityInfluencer', () => {
  ...
  it('should successfully retrieve a opportunityInfluencer and return status 200 with the opportunityInfluencer data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock opportunityInfluencer data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityInfluencerData),
    );
...
describe('updateOpportunityInfluencer', () => {
  ...
  it('should successfully update a opportunityInfluencer and return status 200 with the updated opportunityInfluencer data', async () => {
    ...
    prisma.opportunityInfluencer.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.opportunityInfluencer.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.opportunityInfluencer.updateMany.mockRejectedValue(databaseError);
