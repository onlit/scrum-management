...
describe('createCompanyInTerritory', () => {
  ...
  it('should successfully create a companyInTerritory and return status 201 with the created object', async () => {
    ...
    const mockCompanyInTerritoryData = {
      territoryId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCompanyInTerritoryData),
    ); // Check if the created companyInTerritory data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCompanyInTerritory(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCompanyInTerritory` function.
describe('getAllCompanyInTerritory', () => {
  ...
  it('successfully retrieves companyInTerritorys', async () => {
    ...
    // Invoke the `getAllCompanyInTerritory` function with the mocked request and response objects.
    await getAllCompanyInTerritory(req, res, next);
...
describe('getCompanyInTerritory', () => {
  ...
  it('should successfully retrieve a companyInTerritory and return status 200 with the companyInTerritory data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock companyInTerritory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCompanyInTerritoryData),
    );
...
describe('updateCompanyInTerritory', () => {
  ...
  it('should successfully update a companyInTerritory and return status 200 with the updated companyInTerritory data', async () => {
    ...
    prisma.companyInTerritory.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.companyInTerritory.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.companyInTerritory.updateMany.mockRejectedValue(databaseError);
