...
describe('createPersonRelationshipHistory', () => {
  ...
  it('should successfully create a personRelationshipHistory and return status 201 with the created object', async () => {
    ...
    const mockPersonRelationshipHistoryData = {
      personRelationshipId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      notes: 'd_compute_test_notes',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonRelationshipHistoryData),
    ); // Check if the created personRelationshipHistory data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(
      createPersonRelationshipHistory(req, res, next),
    ).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllPersonRelationshipHistory` function.
describe('getAllPersonRelationshipHistory', () => {
  ...
  it('successfully retrieves personRelationshipHistorys', async () => {
    ...
    // Invoke the `getAllPersonRelationshipHistory` function with the mocked request and response objects.
    await getAllPersonRelationshipHistory(req, res, next);
...
describe('getPersonRelationshipHistory', () => {
  ...
  it('should successfully retrieve a personRelationshipHistory and return status 200 with the personRelationshipHistory data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock personRelationshipHistory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonRelationshipHistoryData),
    );
  ...
  it('should return a 404 error if the personRelationshipHistory is not found', async () => {
    ...
    await expect(
      getPersonRelationshipHistory(req, res, next),
    ).rejects.toThrow();
  });
...
describe('updatePersonRelationshipHistory', () => {
  ...
  it('should successfully update a personRelationshipHistory and return status 200 with the updated personRelationshipHistory data', async () => {
    ...
    prisma.personRelationshipHistory.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.personRelationshipHistory.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.personRelationshipHistory.updateMany.mockRejectedValue(
      databaseError,
    );
