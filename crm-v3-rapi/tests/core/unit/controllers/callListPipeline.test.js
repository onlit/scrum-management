...
} = require('#core/controllers/callListPipeline.controller.core.js');
...
describe('createCallListPipeline', () => {
  ...
  it('should successfully create a callListPipeline and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallListPipelineData),
    ); // Check if the created callListPipeline data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCallListPipeline(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCallListPipeline` function.
describe('getAllCallListPipeline', () => {
  ...
  it('successfully retrieves callListPipelines', async () => {
    ...
    // Invoke the `getAllCallListPipeline` function with the mocked request and response objects.
    await getAllCallListPipeline(req, res, next);
...
describe('getCallListPipeline', () => {
  ...
  it('should successfully retrieve a callListPipeline and return status 200 with the callListPipeline data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock callListPipeline data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallListPipelineData),
    );
...
describe('updateCallListPipeline', () => {
  ...
  it('should successfully update a callListPipeline and return status 200 with the updated callListPipeline data', async () => {
    ...
    prisma.callListPipeline.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.callListPipeline.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callListPipeline.updateMany.mockRejectedValue(databaseError);
...
describe('deleteCallListPipeline', () => {
  ...
  it('should successfully delete a callListPipeline and return status 200 with the deleted ID', async () => {
    ...
    prisma.callList.updateMany.mockResolvedValue({ count: 0 });
    prisma.callListPipelineStage.updateMany.mockResolvedValue({ count: 0 });
