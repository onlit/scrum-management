...
// Import the createPerson function from the controller to be tested
const {
  createPerson,
  getAllPerson,
  getPerson,
...
// Mock the Prisma client's person model to prevent actual database operations during testing
...
// Define a test suite for the createPerson functionality
describe('createPerson', () => {
  ...
  // Test case: successfully creating a person
  it('should successfully create a person and return status 201 with the created object', async () => {
    // Define mock data for the person to be created
    const mockPersonData = {
      firstName: 'd_compute_test_firstName',
      hasWhatsapp: true,
      email: 'd_compute_test_email@test.example.com',
    };
    ...
    const req = {
      ...
      body: mockPersonData, // Body contains the mock person data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock person data
    prisma.person.create.mockResolvedValue(mockPersonData);

    // Execute the createPerson function with the mocked request and response
    await createPerson(req, res, next);

    // Assert that the person was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonData),
    ); // Check if the created person data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid person data to simulate a validation failure
    ...
    const req = {
      ...
      body: invalidPersonData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createPerson function and expect it to throw an error due to validation failure
    await expect(createPerson(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.person.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createPerson function and expect it to throw an error due to database operation failure
    await expect(createPerson(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllPerson` function.
describe('getAllPerson', () => {
  ...
  // Defines a test case that verifies the successful retrieval of persons.
  it('successfully retrieves persons', async () => {
    ...
    // when it successfully fetches persons from the database.
      ...
      results: [
        {
          ...
          name: 'Person One',
          description: 'Description for Person One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllPerson` function with the mocked request and response objects.
    await getAllPerson(req, res, next);
...
// Describe block defines a test suite for the `getPerson` function.
describe('getPerson', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
  ...
  // First test case: it checks if the function successfully retrieves a person
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a person and return status 200 with the person data', async () => {
    // Mock data representing a person returned from the database.
    const mockPersonData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test Person',
    };

    // Mock the Prisma call to find the first person that matches criteria.
    // It's resolved with `mockPersonData`.
    prisma.person.findFirst.mockResolvedValue(mockPersonData);
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock person data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonData),
    );
  ...
  // Second test case: it checks the behavior when a person is not found.
  // The expected behavior is to throw an error, indicating the person is not found.
  it('should return a 404 error if the person is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the person.
    prisma.person.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent person ID.
    ...
    await expect(getPerson(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.person.findFirst.mockRejectedValue(new Error('Database error'));
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any person ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updatePerson', () => {
  ...
  // This test verifies that the `updatePerson` function can successfully update a person
  // in the database and return the updated person data with a 200 HTTP status code.
  it('should successfully update a person and return status 200 with the updated person data', async () => {
    // Mock data for the test, including the person ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      firstName: 'd_compute_update_firstName',
    };
    // The expected result after updating the person.
    ...
    prisma.person.findFirst.mockResolvedValue(mockUpdatedPerson);
    ...
    expect(prisma.person.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updatePerson` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.person.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(updatePerson(req, {}, next)).rejects.toThrow();
  });

  // This test checks that the `updatePerson` function transforms the keys of the request body
...
// This describes a test suite for the `deletePerson` function.
describe('deletePerson', () => {
  ...
  it('should successfully delete a person and return status 200 with the deleted ID', async () => {
    // Mock ID of the person to be deleted.
    ...
    prisma.companyContact.updateMany.mockResolvedValue({ count: 0 });
    prisma.customerEnquiry.updateMany.mockResolvedValue({ count: 0 });
    prisma.personHistory.updateMany.mockResolvedValue({ count: 0 });
    ...
    // Mocks the HTTP request object, including the person ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: {
        id: 'user-123',
    ...
    // containing the ID of the deleted person.
