...
describe('createSalesPersonTarget', () => {
  ...
  it('should successfully create a salesPersonTarget and return status 201 with the created object', async () => {
    ...
    const mockSalesPersonTargetData = {
      pipelineId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      pipelineStageId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockSalesPersonTargetData),
    ); // Check if the created salesPersonTarget data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createSalesPersonTarget(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllSalesPersonTarget` function.
describe('getAllSalesPersonTarget', () => {
  ...
  it('successfully retrieves salesPersonTargets', async () => {
    ...
    // Invoke the `getAllSalesPersonTarget` function with the mocked request and response objects.
    await getAllSalesPersonTarget(req, res, next);
...
describe('getSalesPersonTarget', () => {
  ...
  it('should successfully retrieve a salesPersonTarget and return status 200 with the salesPersonTarget data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock salesPersonTarget data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockSalesPersonTargetData),
    );
...
describe('updateSalesPersonTarget', () => {
  ...
  it('should successfully update a salesPersonTarget and return status 200 with the updated salesPersonTarget data', async () => {
    ...
    prisma.salesPersonTarget.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.salesPersonTarget.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.salesPersonTarget.updateMany.mockRejectedValue(databaseError);
...
describe('deleteSalesPersonTarget', () => {
  ...
  it('should successfully delete a salesPersonTarget and return status 200 with the deleted ID', async () => {
    ...
    prisma.targetActualHistory.updateMany.mockResolvedValue({ count: 0 });
