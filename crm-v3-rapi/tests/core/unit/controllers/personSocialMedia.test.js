...
describe('createPersonSocialMedia', () => {
  ...
  it('should successfully create a personSocialMedia and return status 201 with the created object', async () => {
    ...
    const mockPersonSocialMediaData = {
      ...
      username: 'd_compute_test_username',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonSocialMediaData),
    ); // Check if the created personSocialMedia data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createPersonSocialMedia(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllPersonSocialMedia` function.
describe('getAllPersonSocialMedia', () => {
  ...
  it('successfully retrieves personSocialMedias', async () => {
    ...
    // Invoke the `getAllPersonSocialMedia` function with the mocked request and response objects.
    await getAllPersonSocialMedia(req, res, next);
...
describe('getPersonSocialMedia', () => {
  ...
  it('should successfully retrieve a personSocialMedia and return status 200 with the personSocialMedia data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock personSocialMedia data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonSocialMediaData),
    );
...
describe('updatePersonSocialMedia', () => {
  ...
  it('should successfully update a personSocialMedia and return status 200 with the updated personSocialMedia data', async () => {
    ...
    prisma.personSocialMedia.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.personSocialMedia.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.personSocialMedia.updateMany.mockRejectedValue(databaseError);
