...
describe('createCustomerEnquiryStatus', () => {
  ...
  it('should successfully create a customerEnquiryStatus and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCustomerEnquiryStatusData),
    ); // Check if the created customerEnquiryStatus data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCustomerEnquiryStatus(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCustomerEnquiryStatus` function.
describe('getAllCustomerEnquiryStatus', () => {
  ...
  it('successfully retrieves customerEnquiryStatuss', async () => {
    ...
    // Invoke the `getAllCustomerEnquiryStatus` function with the mocked request and response objects.
    await getAllCustomerEnquiryStatus(req, res, next);
...
describe('getCustomerEnquiryStatus', () => {
  ...
  it('should successfully retrieve a customerEnquiryStatus and return status 200 with the customerEnquiryStatus data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock customerEnquiryStatus data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCustomerEnquiryStatusData),
    );
...
describe('updateCustomerEnquiryStatus', () => {
  ...
  it('should successfully update a customerEnquiryStatus and return status 200 with the updated customerEnquiryStatus data', async () => {
    ...
    prisma.customerEnquiryStatus.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.customerEnquiryStatus.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.customerEnquiryStatus.updateMany.mockRejectedValue(databaseError);
...
describe('deleteCustomerEnquiryStatus', () => {
  ...
  it('should successfully delete a customerEnquiryStatus and return status 200 with the deleted ID', async () => {
    ...
    prisma.customerEnquiry.updateMany.mockResolvedValue({ count: 0 });
