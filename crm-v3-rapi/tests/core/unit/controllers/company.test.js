...
describe('createCompany', () => {
  ...
  it('should successfully create a company and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCompanyData),
    ); // Check if the created company data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCompany(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCompany` function.
describe('getAllCompany', () => {
  ...
  it('successfully retrieves companys', async () => {
    ...
    // Invoke the `getAllCompany` function with the mocked request and response objects.
    await getAllCompany(req, res, next);
...
describe('getCompany', () => {
  ...
  it('should successfully retrieve a company and return status 200 with the company data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock company data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCompanyData),
    );
...
describe('updateCompany', () => {
  ...
  it('should successfully update a company and return status 200 with the updated company data', async () => {
    ...
    prisma.company.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.company.findFirst.mockResolvedValue(mockUpdatedCompany);
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.company.updateMany.mockRejectedValue(databaseError);
...
describe('deleteCompany', () => {
  ...
  it('should successfully delete a company and return status 200 with the deleted ID', async () => {
    ...
    prisma.accountManagerInCompany.updateMany.mockResolvedValue({ count: 0 });
    prisma.companySocialMedia.updateMany.mockResolvedValue({ count: 0 });
    prisma.company.updateMany.mockResolvedValue({ count: 0 });
    prisma.companyHistory.updateMany.mockResolvedValue({ count: 0 });
    prisma.companyInTerritory.updateMany.mockResolvedValue({ count: 0 });
    prisma.companySpin.updateMany.mockResolvedValue({ count: 0 });
