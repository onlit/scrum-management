...
describe('createOpportunityHistory', () => {
  ...
  it('should successfully create a opportunityHistory and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityHistoryData),
    ); // Check if the created opportunityHistory data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createOpportunityHistory(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllOpportunityHistory` function.
describe('getAllOpportunityHistory', () => {
  ...
  it('successfully retrieves opportunityHistorys', async () => {
    ...
    // Invoke the `getAllOpportunityHistory` function with the mocked request and response objects.
    await getAllOpportunityHistory(req, res, next);
...
describe('getOpportunityHistory', () => {
  ...
  it('should successfully retrieve a opportunityHistory and return status 200 with the opportunityHistory data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock opportunityHistory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityHistoryData),
    );
...
describe('updateOpportunityHistory', () => {
  ...
  it('should successfully update a opportunityHistory and return status 200 with the updated opportunityHistory data', async () => {
    ...
    prisma.opportunityHistory.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.opportunityHistory.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.opportunityHistory.updateMany.mockRejectedValue(databaseError);
