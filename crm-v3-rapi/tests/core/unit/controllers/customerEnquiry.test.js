...
describe('createCustomerEnquiry', () => {
  ...
  it('should successfully create a customerEnquiry and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCustomerEnquiryData),
    ); // Check if the created customerEnquiry data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCustomerEnquiry(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCustomerEnquiry` function.
describe('getAllCustomerEnquiry', () => {
  ...
  it('successfully retrieves customerEnquirys', async () => {
    ...
    // Invoke the `getAllCustomerEnquiry` function with the mocked request and response objects.
    await getAllCustomerEnquiry(req, res, next);
...
describe('getCustomerEnquiry', () => {
  ...
  it('should successfully retrieve a customerEnquiry and return status 200 with the customerEnquiry data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock customerEnquiry data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCustomerEnquiryData),
    );
...
describe('updateCustomerEnquiry', () => {
  ...
  it('should successfully update a customerEnquiry and return status 200 with the updated customerEnquiry data', async () => {
    ...
    const mockUpdatedCustomerEnquiry = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.customerEnquiry.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.customerEnquiry.findFirst.mockResolvedValue(
      mockUpdatedCustomerEnquiry,
    );
    ...
    expect(prisma.customerEnquiry.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.customerEnquiry.updateMany.mockRejectedValue(databaseError);
