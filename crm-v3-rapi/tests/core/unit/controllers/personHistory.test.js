...
describe('createPersonHistory', () => {
  ...
  it('should successfully create a personHistory and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonHistoryData),
    ); // Check if the created personHistory data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createPersonHistory(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllPersonHistory` function.
describe('getAllPersonHistory', () => {
  ...
  it('successfully retrieves personHistorys', async () => {
    ...
    // Invoke the `getAllPersonHistory` function with the mocked request and response objects.
    await getAllPersonHistory(req, res, next);
...
describe('getPersonHistory', () => {
  ...
  it('should successfully retrieve a personHistory and return status 200 with the personHistory data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock personHistory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockPersonHistoryData),
    );
...
describe('updatePersonHistory', () => {
  ...
  it('should successfully update a personHistory and return status 200 with the updated personHistory data', async () => {
    ...
    const mockUpdatedPersonHistory = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.personHistory.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.personHistory.findFirst.mockResolvedValue(mockUpdatedPersonHistory);
    ...
    expect(prisma.personHistory.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.personHistory.updateMany.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    prisma.personHistory.findFirst.mockResolvedValue({
      id,
