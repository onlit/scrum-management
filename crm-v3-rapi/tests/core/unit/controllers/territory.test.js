...
describe('createTerritory', () => {
  ...
  it('should successfully create a territory and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockTerritoryData),
    ); // Check if the created territory data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createTerritory(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllTerritory` function.
describe('getAllTerritory', () => {
  ...
  it('successfully retrieves territorys', async () => {
    ...
    // Invoke the `getAllTerritory` function with the mocked request and response objects.
    await getAllTerritory(req, res, next);
...
describe('getTerritory', () => {
  ...
  it('should successfully retrieve a territory and return status 200 with the territory data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock territory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockTerritoryData),
    );
...
describe('updateTerritory', () => {
  ...
  it('should successfully update a territory and return status 200 with the updated territory data', async () => {
    ...
    prisma.territory.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.territory.findFirst.mockResolvedValue(mockUpdatedTerritory);
    ...
    expect(prisma.territory.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.territory.updateMany.mockRejectedValue(databaseError);
...
describe('deleteTerritory', () => {
  ...
  it('should successfully delete a territory and return status 200 with the deleted ID', async () => {
    ...
    prisma.territoryOwner.updateMany.mockResolvedValue({ count: 0 });
