...
// Import the createOpportunityPipeline function from the controller to be tested
const {
...
// Mock the Prisma client's opportunityPipeline model to prevent actual database operations during testing
...
// Define a test suite for the createOpportunityPipeline functionality
describe('createOpportunityPipeline', () => {
  ...
  // Test case: successfully creating a opportunityPipeline
  it('should successfully create a opportunityPipeline and return status 201 with the created object', async () => {
    // Define mock data for the opportunityPipeline to be created
    const mockOpportunityPipelineData = {
    ...
    const req = {
      ...
      body: mockOpportunityPipelineData, // Body contains the mock opportunityPipeline data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock opportunityPipeline data
    prisma.opportunityPipeline.create.mockResolvedValue(
      mockOpportunityPipelineData,
    );

    // Execute the createOpportunityPipeline function with the mocked request and response
    await createOpportunityPipeline(req, res, next);

    // Assert that the opportunityPipeline was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityPipelineData),
    ); // Check if the created opportunityPipeline data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid opportunityPipeline data to simulate a validation failure
    const invalidOpportunityPipelineData = {
    ...
    const req = {
      ...
      body: invalidOpportunityPipelineData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createOpportunityPipeline function and expect it to throw an error due to validation failure
    await expect(createOpportunityPipeline(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.opportunityPipeline.create.mockRejectedValue(
    ...
    // Execute the createOpportunityPipeline function and expect it to throw an error due to database operation failure
    await expect(createOpportunityPipeline(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllOpportunityPipeline` function.
describe('getAllOpportunityPipeline', () => {
  ...
  // Defines a test case that verifies the successful retrieval of opportunityPipelines.
  it('successfully retrieves opportunityPipelines', async () => {
    ...
    // when it successfully fetches opportunityPipelines from the database.
      ...
      results: [
        {
          ...
          name: 'OpportunityPipeline One',
          description: 'Description for OpportunityPipeline One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllOpportunityPipeline` function with the mocked request and response objects.
    await getAllOpportunityPipeline(req, res, next);
...
// Describe block defines a test suite for the `getOpportunityPipeline` function.
describe('getOpportunityPipeline', () => {
  ...
  // First test case: it checks if the function successfully retrieves a opportunityPipeline
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a opportunityPipeline and return status 200 with the opportunityPipeline data', async () => {
    // Mock data representing a opportunityPipeline returned from the database.
    const mockOpportunityPipelineData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test OpportunityPipeline',
    };

    // Mock the Prisma call to find the first opportunityPipeline that matches criteria.
    // It's resolved with `mockOpportunityPipelineData`.
    prisma.opportunityPipeline.findFirst.mockResolvedValue(
      mockOpportunityPipelineData,
    );
    ...
    await getOpportunityPipeline(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock opportunityPipeline data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityPipelineData),
    );
  ...
  // Second test case: it checks the behavior when a opportunityPipeline is not found.
  // The expected behavior is to throw an error, indicating the opportunityPipeline is not found.
  it('should return a 404 error if the opportunityPipeline is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the opportunityPipeline.
    prisma.opportunityPipeline.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent opportunityPipeline ID.
    ...
    await expect(getOpportunityPipeline(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.opportunityPipeline.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any opportunityPipeline ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateOpportunityPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateOpportunityPipeline` function can successfully update a opportunityPipeline
  // in the database and return the updated opportunityPipeline data with a 200 HTTP status code.
  it('should successfully update a opportunityPipeline and return status 200 with the updated opportunityPipeline data', async () => {
    // Mock data for the test, including the opportunityPipeline ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'd_compute_update_name',
    };
    // The expected result after updating the opportunityPipeline.
    const mockUpdatedOpportunityPipeline = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.opportunityPipeline.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.opportunityPipeline.findFirst.mockResolvedValue(
      mockUpdatedOpportunityPipeline,
    );
    ...
    await updateOpportunityPipeline(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    // Verify that the Prisma updateMany method was called.
    expect(prisma.opportunityPipeline.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updateOpportunityPipeline` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.opportunityPipeline.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(updateOpportunityPipeline(req, {}, next)).rejects.toThrow();
  });

  // This test checks that the `updateOpportunityPipeline` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    await updateOpportunityPipeline(req, res);
...
// This describes a test suite for the `deleteOpportunityPipeline` function.
describe('deleteOpportunityPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a opportunityPipeline and return status 200 with the deleted ID', async () => {
    // Mock ID of the opportunityPipeline to be deleted.
    ...
    // Mocks the HTTP request object, including the opportunityPipeline ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: {
        id: 'user-123',
    ...
    await deleteOpportunityPipeline(req, res, next);
    ...
    // containing the ID of the deleted opportunityPipeline.
