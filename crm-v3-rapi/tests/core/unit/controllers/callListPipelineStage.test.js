...
describe('createCallListPipelineStage', () => {
  ...
  it('should successfully create a callListPipelineStage and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallListPipelineStageData),
    ); // Check if the created callListPipelineStage data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCallListPipelineStage(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCallListPipelineStage` function.
describe('getAllCallListPipelineStage', () => {
  ...
  it('successfully retrieves callListPipelineStages', async () => {
    ...
    // Invoke the `getAllCallListPipelineStage` function with the mocked request and response objects.
    await getAllCallListPipelineStage(req, res, next);
...
describe('getCallListPipelineStage', () => {
  ...
  it('should successfully retrieve a callListPipelineStage and return status 200 with the callListPipelineStage data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock callListPipelineStage data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallListPipelineStageData),
    );
...
describe('updateCallListPipelineStage', () => {
  ...
  it('should successfully update a callListPipelineStage and return status 200 with the updated callListPipelineStage data', async () => {
    ...
    prisma.callListPipelineStage.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.callListPipelineStage.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callListPipelineStage.updateMany.mockRejectedValue(databaseError);
...
describe('deleteCallListPipelineStage', () => {
  ...
  it('should successfully delete a callListPipelineStage and return status 200 with the deleted ID', async () => {
    ...
    prisma.callSchedule.updateMany.mockResolvedValue({ count: 0 });
    prisma.callHistory.updateMany.mockResolvedValue({ count: 0 });
