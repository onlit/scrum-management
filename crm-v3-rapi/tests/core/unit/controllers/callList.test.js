...
describe('createCallList', () => {
  ...
  it('should successfully create a callList and return status 201 with the created object', async () => {
    ...
    const mockCallListData = {
      callListPipelineId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallListData),
    ); // Check if the created callList data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCallList(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCallList` function.
describe('getAllCallList', () => {
  ...
  it('successfully retrieves callLists', async () => {
    ...
    // Invoke the `getAllCallList` function with the mocked request and response objects.
    await getAllCallList(req, res, next);
...
describe('getCallList', () => {
  ...
  it('should successfully retrieve a callList and return status 200 with the callList data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock callList data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallListData),
    );
...
describe('updateCallList', () => {
  ...
  it('should successfully update a callList and return status 200 with the updated callList data', async () => {
    ...
    expect(prisma.callList.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callList.updateMany.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    prisma.callList.findFirst.mockResolvedValue({
      id,
