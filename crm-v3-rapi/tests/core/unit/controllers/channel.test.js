...
describe('createChannel', () => {
  ...
  it('should successfully create a channel and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockChannelData),
    ); // Check if the created channel data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createChannel(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllChannel` function.
describe('getAllChannel', () => {
  ...
  it('successfully retrieves channels', async () => {
    ...
    // Invoke the `getAllChannel` function with the mocked request and response objects.
    await getAllChannel(req, res, next);
...
describe('getChannel', () => {
  ...
  it('should successfully retrieve a channel and return status 200 with the channel data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock channel data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockChannelData),
    );
...
describe('updateChannel', () => {
  ...
  it('should successfully update a channel and return status 200 with the updated channel data', async () => {
    ...
    prisma.channel.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.channel.findFirst.mockResolvedValue(mockUpdatedChannel);
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.channel.updateMany.mockRejectedValue(databaseError);
