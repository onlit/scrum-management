...
describe('createCallSchedule', () => {
  ...
  it('should successfully create a callSchedule and return status 201 with the created object', async () => {
    ...
    const mockCallScheduleData = {
      callListPipelineStageId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallScheduleData),
    ); // Check if the created callSchedule data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCallSchedule(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCallSchedule` function.
describe('getAllCallSchedule', () => {
  ...
  it('successfully retrieves callSchedules', async () => {
    ...
    // Invoke the `getAllCallSchedule` function with the mocked request and response objects.
    await getAllCallSchedule(req, res, next);
...
describe('getCallSchedule', () => {
  ...
  it('should successfully retrieve a callSchedule and return status 200 with the callSchedule data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock callSchedule data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallScheduleData),
    );
...
describe('updateCallSchedule', () => {
  ...
  it('should successfully update a callSchedule and return status 200 with the updated callSchedule data', async () => {
    ...
    const mockUpdatedCallSchedule = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.callSchedule.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.callSchedule.findFirst.mockResolvedValue(mockUpdatedCallSchedule);
    ...
    expect(prisma.callSchedule.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callSchedule.updateMany.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    prisma.callSchedule.findFirst.mockResolvedValue({
      id,
