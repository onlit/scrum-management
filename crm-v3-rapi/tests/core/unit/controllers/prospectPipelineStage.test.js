...
// Import the createProspectPipelineStage function from the controller to be tested
const {
...
// Mock the Prisma client's prospectPipelineStage model to prevent actual database operations during testing
...
// Define a test suite for the createProspectPipelineStage functionality
describe('createProspectPipelineStage', () => {
  ...
  // Test case: successfully creating a prospectPipelineStage
  it('should successfully create a prospectPipelineStage and return status 201 with the created object', async () => {
    // Define mock data for the prospectPipelineStage to be created
    const mockProspectPipelineStageData = {
      order: 42,
      confidence: 42,
      rottingDays: 42,
      conversion: 42,
    ...
    const req = {
      ...
      body: mockProspectPipelineStageData, // Body contains the mock prospectPipelineStage data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock prospectPipelineStage data
    prisma.prospectPipelineStage.create.mockResolvedValue(
      mockProspectPipelineStageData,
    );

    // Execute the createProspectPipelineStage function with the mocked request and response
    await createProspectPipelineStage(req, res, next);

    // Assert that the prospectPipelineStage was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockProspectPipelineStageData),
    ); // Check if the created prospectPipelineStage data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid prospectPipelineStage data to simulate a validation failure
    const invalidProspectPipelineStageData = {
    ...
    const req = {
      ...
      body: invalidProspectPipelineStageData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createProspectPipelineStage function and expect it to throw an error due to validation failure
    await expect(createProspectPipelineStage(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.prospectPipelineStage.create.mockRejectedValue(
    ...
    // Execute the createProspectPipelineStage function and expect it to throw an error due to database operation failure
    await expect(createProspectPipelineStage(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllProspectPipelineStage` function.
describe('getAllProspectPipelineStage', () => {
  ...
  // Defines a test case that verifies the successful retrieval of prospectPipelineStages.
  it('successfully retrieves prospectPipelineStages', async () => {
    ...
    // when it successfully fetches prospectPipelineStages from the database.
      ...
      results: [
        {
          ...
          name: 'ProspectPipelineStage One',
          description: 'Description for ProspectPipelineStage One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllProspectPipelineStage` function with the mocked request and response objects.
    await getAllProspectPipelineStage(req, res, next);
...
// Describe block defines a test suite for the `getProspectPipelineStage` function.
describe('getProspectPipelineStage', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a prospectPipelineStage
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a prospectPipelineStage and return status 200 with the prospectPipelineStage data', async () => {
    // Mock data representing a prospectPipelineStage returned from the database.
    const mockProspectPipelineStageData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test ProspectPipelineStage',
    };

    // Mock the Prisma call to find the first prospectPipelineStage that matches criteria.
    // It's resolved with `mockProspectPipelineStageData`.
    prisma.prospectPipelineStage.findFirst.mockResolvedValue(
      mockProspectPipelineStageData,
    );
    ...
    await getProspectPipelineStage(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock prospectPipelineStage data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockProspectPipelineStageData),
    );
  ...
  // Second test case: it checks the behavior when a prospectPipelineStage is not found.
  // The expected behavior is to throw an error, indicating the prospectPipelineStage is not found.
  it('should return a 404 error if the prospectPipelineStage is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the prospectPipelineStage.
    prisma.prospectPipelineStage.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent prospectPipelineStage ID.
    ...
    await expect(getProspectPipelineStage(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.prospectPipelineStage.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any prospectPipelineStage ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateProspectPipelineStage', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateProspectPipelineStage` function can successfully update a prospectPipelineStage
  // in the database and return the updated prospectPipelineStage data with a 200 HTTP status code.
  it('should successfully update a prospectPipelineStage and return status 200 with the updated prospectPipelineStage data', async () => {
    // Mock data for the test, including the prospectPipelineStage ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      order: 42,
    };
    // The expected result after updating the prospectPipelineStage.
    const mockUpdatedProspectPipelineStage = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.prospectPipelineStage.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.prospectPipelineStage.findFirst.mockResolvedValue(
      mockUpdatedProspectPipelineStage,
    );
    ...
    await updateProspectPipelineStage(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    // Verify that the Prisma updateMany method was called.
    expect(prisma.prospectPipelineStage.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updateProspectPipelineStage` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.prospectPipelineStage.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(updateProspectPipelineStage(req, {}, next)).rejects.toThrow();
  });

  // This test checks that the `updateProspectPipelineStage` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    await updateProspectPipelineStage(req, res);
...
// This describes a test suite for the `deleteProspectPipelineStage` function.
describe('deleteProspectPipelineStage', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a prospectPipelineStage and return status 200 with the deleted ID', async () => {
    // Mock ID of the prospectPipelineStage to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    prisma.prospect.updateMany.mockResolvedValue({ count: 0 });
    prisma.prospectPipelineStage.updateMany.mockResolvedValue({ count: 0 });
    ...
    // Mocks the HTTP request object, including the prospectPipelineStage ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: {
        id: 'user-123',
    ...
    await deleteProspectPipelineStage(req, res, next);
    ...
    // containing the ID of the deleted prospectPipelineStage.
