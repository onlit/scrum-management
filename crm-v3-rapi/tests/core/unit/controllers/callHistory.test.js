...
describe('createCallHistory', () => {
  ...
  it('should successfully create a callHistory and return status 201 with the created object', async () => {
    ...
    const mockCallHistoryData = {
      ...
      outcome: 'd_compute_test_outcome',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallHistoryData),
    ); // Check if the created callHistory data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCallHistory(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCallHistory` function.
describe('getAllCallHistory', () => {
  ...
  it('successfully retrieves callHistorys', async () => {
    ...
    // Invoke the `getAllCallHistory` function with the mocked request and response objects.
    await getAllCallHistory(req, res, next);
...
describe('getCallHistory', () => {
  ...
  it('should successfully retrieve a callHistory and return status 200 with the callHistory data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock callHistory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCallHistoryData),
    );
...
describe('updateCallHistory', () => {
  ...
  it('should successfully update a callHistory and return status 200 with the updated callHistory data', async () => {
    ...
    expect(prisma.callHistory.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.callHistory.updateMany.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    prisma.callHistory.findFirst.mockResolvedValue({
      id,
