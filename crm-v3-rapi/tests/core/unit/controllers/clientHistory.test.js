...
describe('createClientHistory', () => {
  ...
  it('should successfully create a clientHistory and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockClientHistoryData),
    ); // Check if the created clientHistory data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createClientHistory(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllClientHistory` function.
describe('getAllClientHistory', () => {
  ...
  it('successfully retrieves clientHistorys', async () => {
    ...
    // Invoke the `getAllClientHistory` function with the mocked request and response objects.
    await getAllClientHistory(req, res, next);
...
describe('getClientHistory', () => {
  ...
  it('should successfully retrieve a clientHistory and return status 200 with the clientHistory data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock clientHistory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockClientHistoryData),
    );
...
describe('updateClientHistory', () => {
  ...
  it('should successfully update a clientHistory and return status 200 with the updated clientHistory data', async () => {
    ...
    const mockUpdatedClientHistory = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.clientHistory.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.clientHistory.findFirst.mockResolvedValue(mockUpdatedClientHistory);
    ...
    expect(prisma.clientHistory.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.clientHistory.updateMany.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    prisma.clientHistory.findFirst.mockResolvedValue({
      id,
