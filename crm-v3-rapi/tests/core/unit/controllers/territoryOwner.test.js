...
describe('createTerritoryOwner', () => {
  ...
  it('should successfully create a territoryOwner and return status 201 with the created object', async () => {
    ...
    const mockTerritoryOwnerData = {
      salesPersonId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockTerritoryOwnerData),
    ); // Check if the created territoryOwner data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createTerritoryOwner(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllTerritoryOwner` function.
describe('getAllTerritoryOwner', () => {
  ...
  it('successfully retrieves territoryOwners', async () => {
    ...
    // Invoke the `getAllTerritoryOwner` function with the mocked request and response objects.
    await getAllTerritoryOwner(req, res, next);
...
describe('getTerritoryOwner', () => {
  ...
  it('should successfully retrieve a territoryOwner and return status 200 with the territoryOwner data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock territoryOwner data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockTerritoryOwnerData),
    );
...
describe('updateTerritoryOwner', () => {
  ...
  it('should successfully update a territoryOwner and return status 200 with the updated territoryOwner data', async () => {
    ...
    const mockUpdatedTerritoryOwner = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.territoryOwner.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.territoryOwner.findFirst.mockResolvedValue(
      mockUpdatedTerritoryOwner,
    );
    ...
    expect(prisma.territoryOwner.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.territoryOwner.updateMany.mockRejectedValue(databaseError);
