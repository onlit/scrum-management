...
// Import the createProspectPipeline function from the controller to be tested
const {
...
// Mock the Prisma client's prospectPipeline model to prevent actual database operations during testing
...
// Define a test suite for the createProspectPipeline functionality
describe('createProspectPipeline', () => {
  ...
  // Test case: successfully creating a prospectPipeline
  it('should successfully create a prospectPipeline and return status 201 with the created object', async () => {
    // Define mock data for the prospectPipeline to be created
    const mockProspectPipelineData = {
    ...
    const req = {
      ...
      body: mockProspectPipelineData, // Body contains the mock prospectPipeline data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock prospectPipeline data
    prisma.prospectPipeline.create.mockResolvedValue(mockProspectPipelineData);

    // Execute the createProspectPipeline function with the mocked request and response
    await createProspectPipeline(req, res, next);

    // Assert that the prospectPipeline was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockProspectPipelineData),
    ); // Check if the created prospectPipeline data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid prospectPipeline data to simulate a validation failure
    const invalidProspectPipelineData = {
    ...
    const req = {
      ...
      body: invalidProspectPipelineData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createProspectPipeline function and expect it to throw an error due to validation failure
    await expect(createProspectPipeline(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.prospectPipeline.create.mockRejectedValue(
    ...
    // Execute the createProspectPipeline function and expect it to throw an error due to database operation failure
    await expect(createProspectPipeline(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllProspectPipeline` function.
describe('getAllProspectPipeline', () => {
  ...
  // Defines a test case that verifies the successful retrieval of prospectPipelines.
  it('successfully retrieves prospectPipelines', async () => {
    ...
    // when it successfully fetches prospectPipelines from the database.
      ...
      results: [
        {
          ...
          name: 'ProspectPipeline One',
          description: 'Description for ProspectPipeline One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllProspectPipeline` function with the mocked request and response objects.
    await getAllProspectPipeline(req, res, next);
...
// Describe block defines a test suite for the `getProspectPipeline` function.
describe('getProspectPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a prospectPipeline
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a prospectPipeline and return status 200 with the prospectPipeline data', async () => {
    // Mock data representing a prospectPipeline returned from the database.
    const mockProspectPipelineData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test ProspectPipeline',
    };

    // Mock the Prisma call to find the first prospectPipeline that matches criteria.
    // It's resolved with `mockProspectPipelineData`.
    prisma.prospectPipeline.findFirst.mockResolvedValue(
      mockProspectPipelineData,
    );
    ...
    await getProspectPipeline(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock prospectPipeline data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockProspectPipelineData),
    );
  ...
  // Second test case: it checks the behavior when a prospectPipeline is not found.
  // The expected behavior is to throw an error, indicating the prospectPipeline is not found.
  it('should return a 404 error if the prospectPipeline is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the prospectPipeline.
    prisma.prospectPipeline.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent prospectPipeline ID.
    ...
    await expect(getProspectPipeline(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.prospectPipeline.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any prospectPipeline ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateProspectPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateProspectPipeline` function can successfully update a prospectPipeline
  // in the database and return the updated prospectPipeline data with a 200 HTTP status code.
  it('should successfully update a prospectPipeline and return status 200 with the updated prospectPipeline data', async () => {
    // Mock data for the test, including the prospectPipeline ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'd_compute_update_name',
    };
    // The expected result after updating the prospectPipeline.
    const mockUpdatedProspectPipeline = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.prospectPipeline.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.prospectPipeline.findFirst.mockResolvedValue(
      mockUpdatedProspectPipeline,
    );
    ...
    await updateProspectPipeline(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    // Verify that the Prisma updateMany method was called.
    expect(prisma.prospectPipeline.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updateProspectPipeline` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.prospectPipeline.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(updateProspectPipeline(req, {}, next)).rejects.toThrow();
  });

  // This test checks that the `updateProspectPipeline` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    await updateProspectPipeline(req, res);
...
// This describes a test suite for the `deleteProspectPipeline` function.
describe('deleteProspectPipeline', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a prospectPipeline and return status 200 with the deleted ID', async () => {
    // Mock ID of the prospectPipeline to be deleted.
    ...
    // Mocks the HTTP request object, including the prospectPipeline ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: {
        id: 'user-123',
    ...
    await deleteProspectPipeline(req, res, next);
    ...
    // containing the ID of the deleted prospectPipeline.
