...
// Import the createOpportunityCategory function from the controller to be tested
const {
...
// Mock the Prisma client's opportunityCategory model to prevent actual database operations during testing
...
// Define a test suite for the createOpportunityCategory functionality
describe('createOpportunityCategory', () => {
  ...
  // Test case: successfully creating a opportunityCategory
  it('should successfully create a opportunityCategory and return status 201 with the created object', async () => {
    // Define mock data for the opportunityCategory to be created
    const mockOpportunityCategoryData = {
    ...
    const req = {
      ...
      body: mockOpportunityCategoryData, // Body contains the mock opportunityCategory data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock opportunityCategory data
    prisma.opportunityCategory.create.mockResolvedValue(
      mockOpportunityCategoryData,
    );

    // Execute the createOpportunityCategory function with the mocked request and response
    await createOpportunityCategory(req, res, next);

    // Assert that the opportunityCategory was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityCategoryData),
    ); // Check if the created opportunityCategory data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid opportunityCategory data to simulate a validation failure
    const invalidOpportunityCategoryData = {
    ...
    const req = {
      ...
      body: invalidOpportunityCategoryData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createOpportunityCategory function and expect it to throw an error due to validation failure
    await expect(createOpportunityCategory(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.opportunityCategory.create.mockRejectedValue(
    ...
    // Execute the createOpportunityCategory function and expect it to throw an error due to database operation failure
    await expect(createOpportunityCategory(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllOpportunityCategory` function.
describe('getAllOpportunityCategory', () => {
  ...
  // Defines a test case that verifies the successful retrieval of opportunityCategorys.
  it('successfully retrieves opportunityCategorys', async () => {
    ...
    // when it successfully fetches opportunityCategorys from the database.
      ...
      results: [
        {
          ...
          name: 'OpportunityCategory One',
          description: 'Description for OpportunityCategory One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllOpportunityCategory` function with the mocked request and response objects.
    await getAllOpportunityCategory(req, res, next);
...
// Describe block defines a test suite for the `getOpportunityCategory` function.
describe('getOpportunityCategory', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // First test case: it checks if the function successfully retrieves a opportunityCategory
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a opportunityCategory and return status 200 with the opportunityCategory data', async () => {
    // Mock data representing a opportunityCategory returned from the database.
    const mockOpportunityCategoryData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test OpportunityCategory',
    };

    // Mock the Prisma call to find the first opportunityCategory that matches criteria.
    // It's resolved with `mockOpportunityCategoryData`.
    prisma.opportunityCategory.findFirst.mockResolvedValue(
      mockOpportunityCategoryData,
    );
    ...
    await getOpportunityCategory(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock opportunityCategory data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityCategoryData),
    );
  ...
  // Second test case: it checks the behavior when a opportunityCategory is not found.
  // The expected behavior is to throw an error, indicating the opportunityCategory is not found.
  it('should return a 404 error if the opportunityCategory is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the opportunityCategory.
    prisma.opportunityCategory.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent opportunityCategory ID.
    ...
    await expect(getOpportunityCategory(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.opportunityCategory.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any opportunityCategory ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateOpportunityCategory', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  // This test verifies that the `updateOpportunityCategory` function can successfully update a opportunityCategory
  // in the database and return the updated opportunityCategory data with a 200 HTTP status code.
  it('should successfully update a opportunityCategory and return status 200 with the updated opportunityCategory data', async () => {
    // Mock data for the test, including the opportunityCategory ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'd_compute_update_name',
    };
    // The expected result after updating the opportunityCategory.
    const mockUpdatedOpportunityCategory = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.opportunityCategory.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.opportunityCategory.findFirst.mockResolvedValue(
      mockUpdatedOpportunityCategory,
    );
    ...
    await updateOpportunityCategory(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    // Verify that the Prisma updateMany method was called.
    expect(prisma.opportunityCategory.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updateOpportunityCategory` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.opportunityCategory.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(updateOpportunityCategory(req, {}, next)).rejects.toThrow();
  });

  // This test checks that the `updateOpportunityCategory` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    await updateOpportunityCategory(req, res);
...
// This describes a test suite for the `deleteOpportunityCategory` function.
describe('deleteOpportunityCategory', () => {
  // `beforeEach` is a Jest hook that runs before each test in this suite.
  // It's used here to clear all mocks to ensure a clean environment for each test.
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('should successfully delete a opportunityCategory and return status 200 with the deleted ID', async () => {
    // Mock ID of the opportunityCategory to be deleted.
    ...
    // Mocks the HTTP request object, including the opportunityCategory ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: {
        id: 'user-123',
    ...
    await deleteOpportunityCategory(req, res, next);
    ...
    // containing the ID of the deleted opportunityCategory.
