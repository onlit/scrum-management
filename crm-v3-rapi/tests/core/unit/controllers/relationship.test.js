...
// Import the createRelationship function from the controller to be tested
const {
  createRelationship,
  getAllRelationship,
  getRelationship,
...
// Mock the Prisma client's relationship model to prevent actual database operations during testing
...
// Define a test suite for the createRelationship functionality
describe('createRelationship', () => {
  ...
  // Test case: successfully creating a relationship
  it('should successfully create a relationship and return status 201 with the created object', async () => {
    // Define mock data for the relationship to be created
    ...
    const req = {
      ...
      body: mockRelationshipData, // Body contains the mock relationship data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock relationship data
    prisma.relationship.create.mockResolvedValue(mockRelationshipData);

    // Execute the createRelationship function with the mocked request and response
    await createRelationship(req, res, next);

    // Assert that the relationship was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockRelationshipData),
    ); // Check if the created relationship data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid relationship data to simulate a validation failure
    ...
    const req = {
      ...
      body: invalidRelationshipData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createRelationship function and expect it to throw an error due to validation failure
    await expect(createRelationship(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.relationship.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createRelationship function and expect it to throw an error due to database operation failure
    await expect(createRelationship(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllRelationship` function.
describe('getAllRelationship', () => {
  ...
  // Defines a test case that verifies the successful retrieval of relationships.
  it('successfully retrieves relationships', async () => {
    ...
    // when it successfully fetches relationships from the database.
      ...
      results: [
        {
          ...
          description: 'Description for Relationship One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllRelationship` function with the mocked request and response objects.
    await getAllRelationship(req, res, next);
...
// Describe block defines a test suite for the `getRelationship` function.
describe('getRelationship', () => {
  ...
  // First test case: it checks if the function successfully retrieves a relationship
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a relationship and return status 200 with the relationship data', async () => {
    // Mock data representing a relationship returned from the database.
    const mockRelationshipData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test Relationship',
    };

    // Mock the Prisma call to find the first relationship that matches criteria.
    // It's resolved with `mockRelationshipData`.
    prisma.relationship.findFirst.mockResolvedValue(mockRelationshipData);
    ...
    await getRelationship(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock relationship data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockRelationshipData),
    );
  ...
  // Second test case: it checks the behavior when a relationship is not found.
  // The expected behavior is to throw an error, indicating the relationship is not found.
  it('should return a 404 error if the relationship is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the relationship.
    prisma.relationship.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent relationship ID.
    ...
    await expect(getRelationship(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.relationship.findFirst.mockRejectedValue(
      new Error('Database error'),
    );
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any relationship ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateRelationship', () => {
  ...
  // This test verifies that the `updateRelationship` function can successfully update a relationship
  // in the database and return the updated relationship data with a 200 HTTP status code.
  it('should successfully update a relationship and return status 200 with the updated relationship data', async () => {
    // Mock data for the test, including the relationship ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      name: 'd_compute_update_name',
    };
    // The expected result after updating the relationship.
    ...
    prisma.relationship.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.relationship.findFirst.mockResolvedValue(mockUpdatedRelationship);
    ...
    expect(prisma.relationship.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updateRelationship` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.relationship.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(updateRelationship(req, {}, next)).rejects.toThrow();
  });

  // This test checks that the `updateRelationship` function transforms the keys of the request body
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    await updateRelationship(req, res);
...
// This describes a test suite for the `deleteRelationship` function.
describe('deleteRelationship', () => {
  ...
  it('should successfully delete a relationship and return status 200 with the deleted ID', async () => {
    // Mock ID of the relationship to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    prisma.personRelationship.updateMany.mockResolvedValue({ count: 0 });
    ...
    // Mocks the HTTP request object, including the relationship ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: {
        id: 'user-123',
    ...
    await deleteRelationship(req, res, next);
    ...
    // containing the ID of the deleted relationship.
