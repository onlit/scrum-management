...
describe('createActionPlan', () => {
  ...
  it('should successfully create a actionPlan and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockActionPlanData),
    ); // Check if the created actionPlan data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createActionPlan(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllActionPlan` function.
describe('getAllActionPlan', () => {
  ...
  it('successfully retrieves actionPlans', async () => {
    ...
    // Invoke the `getAllActionPlan` function with the mocked request and response objects.
    await getAllActionPlan(req, res, next);
...
describe('getActionPlan', () => {
  ...
  it('should successfully retrieve a actionPlan and return status 200 with the actionPlan data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock actionPlan data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockActionPlanData),
    );
...
describe('updateActionPlan', () => {
  ...
  it('should successfully update a actionPlan and return status 200 with the updated actionPlan data', async () => {
    ...
    expect(prisma.actionPlan.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.actionPlan.updateMany.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    prisma.actionPlan.findFirst.mockResolvedValue({
      id,
