...
describe('createOpportunity', () => {
  ...
  it('should successfully create a opportunity and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityData),
    ); // Check if the created opportunity data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createOpportunity(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllOpportunity` function.
describe('getAllOpportunity', () => {
  ...
  it('successfully retrieves opportunitys', async () => {
    ...
    // Invoke the `getAllOpportunity` function with the mocked request and response objects.
    await getAllOpportunity(req, res, next);
...
describe('getOpportunity', () => {
  ...
  it('should successfully retrieve a opportunity and return status 200 with the opportunity data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock opportunity data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockOpportunityData),
    );
...
describe('updateOpportunity', () => {
  ...
  it('should successfully update a opportunity and return status 200 with the updated opportunity data', async () => {
    ...
    prisma.opportunity.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.opportunity.findFirst.mockResolvedValue(mockUpdatedOpportunity);
    ...
    expect(prisma.opportunity.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.opportunity.updateMany.mockRejectedValue(databaseError);
...
describe('deleteOpportunity', () => {
  ...
  it('should successfully delete a opportunity and return status 200 with the deleted ID', async () => {
    ...
    prisma.actionPlan.updateMany.mockResolvedValue({ count: 0 });
    prisma.dataNeeded.updateMany.mockResolvedValue({ count: 0 });
    prisma.opportunityProduct.updateMany.mockResolvedValue({ count: 0 });
    prisma.opportunityHistory.updateMany.mockResolvedValue({ count: 0 });
