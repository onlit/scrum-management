...
describe('createSocialMediaType', () => {
  ...
  it('should successfully create a socialMediaType and return status 201 with the created object', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockSocialMediaTypeData),
    ); // Check if the created socialMediaType data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createSocialMediaType(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllSocialMediaType` function.
describe('getAllSocialMediaType', () => {
  ...
  it('successfully retrieves socialMediaTypes', async () => {
    ...
    // Invoke the `getAllSocialMediaType` function with the mocked request and response objects.
    await getAllSocialMediaType(req, res, next);
...
describe('getSocialMediaType', () => {
  ...
  it('should successfully retrieve a socialMediaType and return status 200 with the socialMediaType data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock socialMediaType data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockSocialMediaTypeData),
    );
...
describe('updateSocialMediaType', () => {
  ...
  it('should successfully update a socialMediaType and return status 200 with the updated socialMediaType data', async () => {
    ...
    const mockUpdatedSocialMediaType = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.socialMediaType.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.socialMediaType.findFirst.mockResolvedValue(
      mockUpdatedSocialMediaType,
    );
    ...
    expect(prisma.socialMediaType.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.socialMediaType.updateMany.mockRejectedValue(databaseError);
...
describe('deleteSocialMediaType', () => {
  ...
  it('should successfully delete a socialMediaType and return status 200 with the deleted ID', async () => {
    ...
    prisma.personSocialMedia.updateMany.mockResolvedValue({ count: 0 });
    prisma.companySocialMedia.updateMany.mockResolvedValue({ count: 0 });
