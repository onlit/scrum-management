...
describe('createCompanySocialMedia', () => {
  ...
  it('should successfully create a companySocialMedia and return status 201 with the created object', async () => {
    ...
    const mockCompanySocialMediaData = {
      url: 'https://example.com/d_compute_test',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCompanySocialMediaData),
    ); // Check if the created companySocialMedia data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createCompanySocialMedia(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllCompanySocialMedia` function.
describe('getAllCompanySocialMedia', () => {
  ...
  it('successfully retrieves companySocialMedias', async () => {
    ...
    // Invoke the `getAllCompanySocialMedia` function with the mocked request and response objects.
    await getAllCompanySocialMedia(req, res, next);
...
describe('getCompanySocialMedia', () => {
  ...
  it('should successfully retrieve a companySocialMedia and return status 200 with the companySocialMedia data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock companySocialMedia data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockCompanySocialMediaData),
    );
...
describe('updateCompanySocialMedia', () => {
  ...
  it('should successfully update a companySocialMedia and return status 200 with the updated companySocialMedia data', async () => {
    ...
    const mockRequestBody = {
      url: 'https://example.com/d_compute_update',
    };
    ...
    prisma.companySocialMedia.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.companySocialMedia.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.companySocialMedia.updateMany.mockRejectedValue(databaseError);
