...
describe('createAccountManagerInCompany', () => {
  ...
  it('should successfully create a accountManagerInCompany and return status 201 with the created object', async () => {
    ...
    const mockAccountManagerInCompanyData = {
      ...
      accountManagerId: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockAccountManagerInCompanyData),
    ); // Check if the created accountManagerInCompany data was returned in the response
...
// Defines a test suite for the `getAllAccountManagerInCompany` function.
describe('getAllAccountManagerInCompany', () => {
  ...
  it('successfully retrieves accountManagerInCompanys', async () => {
    ...
    // Invoke the `getAllAccountManagerInCompany` function with the mocked request and response objects.
    await getAllAccountManagerInCompany(req, res, next);
...
describe('getAccountManagerInCompany', () => {
  ...
  it('should successfully retrieve a accountManagerInCompany and return status 200 with the accountManagerInCompany data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock accountManagerInCompany data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockAccountManagerInCompanyData),
    );
...
describe('updateAccountManagerInCompany', () => {
  ...
  it('should successfully update a accountManagerInCompany and return status 200 with the updated accountManagerInCompany data', async () => {
    ...
    prisma.accountManagerInCompany.updateMany.mockResolvedValue({
      count: 1,
    });
    ...
    expect(prisma.accountManagerInCompany.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.accountManagerInCompany.updateMany.mockRejectedValue(databaseError);
    ...
    await expect(
      updateAccountManagerInCompany(req, {}, next),
    ).rejects.toThrow();
  });
