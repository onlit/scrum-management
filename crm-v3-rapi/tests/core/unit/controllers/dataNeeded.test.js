...
describe('createDataNeeded', () => {
  ...
  it('should successfully create a dataNeeded and return status 201 with the created object', async () => {
    ...
    const mockDataNeededData = {
      whoFrom: 'd_compute_test_whoFrom',
      infoNeeded: 'd_compute_test_infoNeeded',
    };
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockDataNeededData),
    ); // Check if the created dataNeeded data was returned in the response
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    await expect(createDataNeeded(req, res, next)).rejects.toThrow(); // Controller wraps errors, so just check that it throws
  });
});

// Defines a test suite for the `getAllDataNeeded` function.
describe('getAllDataNeeded', () => {
  ...
  it('successfully retrieves dataNeededs', async () => {
    ...
    // Invoke the `getAllDataNeeded` function with the mocked request and response objects.
    await getAllDataNeeded(req, res, next);
...
describe('getDataNeeded', () => {
  ...
  it('should successfully retrieve a dataNeeded and return status 200 with the dataNeeded data', async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock dataNeeded data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockDataNeededData),
    );
...
describe('updateDataNeeded', () => {
  ...
  it('should successfully update a dataNeeded and return status 200 with the updated dataNeeded data', async () => {
    ...
    expect(prisma.dataNeeded.updateMany).toHaveBeenCalled();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.dataNeeded.updateMany.mockRejectedValue(databaseError);
  ...
  it('transforms request body keys to camelCase before updating', async () => {
    ...
    prisma.dataNeeded.findFirst.mockResolvedValue({
      id,
