...
describe('Relationship Integration Tests', () => {
  ...
  describe('POST /api/v1/relationships - Create', () => {
    it('should create a new Relationship and persist to database', async () => {
      ...
      const dbRecord = await prisma.relationship.findUnique({
  ...
  describe('GET /api/v1/relationships - List', () => {
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/relationships?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/relationships/:id - Get One', () => {
    it('should return a specific Relationship by ID', async () => {
      const created = await createRelationship();

      const response = await request(server)
        .get(`/api/v1/relationships/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/relationships/:id - Update', () => {
    it('should update an existing Relationship', async () => {
      ...
      const response = await request(server)
        .put(`/api/v1/relationships/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/relationships/:id - Delete', () => {
    it('should delete an existing Relationship', async () => {
      const created = await createRelationship();

      const response = await request(server)
        .delete(`/api/v1/relationships/${created.id}`)
        .set(authHeaders)
        .expect(200);
