 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for TerritoryOwner CRUD operations.
...
describe('TerritoryOwner Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('POST /api/v1/territory-owners - Create', () => {
    it('should create a new TerritoryOwner and persist to database', async () => {
      ...
      const dbRecord = await prisma.territoryOwner.findUnique({
  ...
  describe('GET /api/v1/territory-owners - List', () => {
    it('should return paginated list of TerritoryOwners', async () => {
      // Create test records
      await createTerritoryOwner();
      await createTerritoryOwner();
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/territory-owners?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/territory-owners/:id - Get One', () => {
    it('should return a specific TerritoryOwner by ID', async () => {
      const created = await createTerritoryOwner();

      const response = await request(server)
        .get(`/api/v1/territory-owners/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/territory-owners/:id - Update', () => {
    it('should update an existing TerritoryOwner', async () => {
      const created = await createTerritoryOwner();
      const updateData = {
      ...
      const response = await request(server)
        .put(`/api/v1/territory-owners/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/territory-owners/:id - Delete', () => {
    it('should delete an existing TerritoryOwner', async () => {
      const created = await createTerritoryOwner();

      const response = await request(server)
        .delete(`/api/v1/territory-owners/${created.id}`)
        .set(authHeaders)
        .expect(200);
