 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for CallList CRUD operations.
...
const {
  buildCallListForApi,
  createCallList,
} = require('#tests/factories/callList.factory.js');

describe('CallList Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('POST /api/v1/call-lists - Create', () => {
    it('should create a new CallList and persist to database', async () => {
      const testData = await buildCallListForApi();
      ...
      const dbRecord = await prisma.callList.findUnique({
  ...
  describe('GET /api/v1/call-lists - List', () => {
    it('should return paginated list of CallLists', async () => {
      // Create test records
      await createCallList();
      await createCallList();

      const response = await request(server)
        .get('/api/v1/call-lists')
        .set(authHeaders)
        .expect(200);
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/call-lists?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/call-lists/:id - Get One', () => {
    it('should return a specific CallList by ID', async () => {
      const created = await createCallList();

      const response = await request(server)
        .get(`/api/v1/call-lists/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/call-lists/:id - Update', () => {
    it('should update an existing CallList', async () => {
      const created = await createCallList();
      const updateData = {
      ...
      const response = await request(server)
        .put(`/api/v1/call-lists/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/call-lists/:id - Delete', () => {
    it('should delete an existing CallList', async () => {
      const created = await createCallList();

      const response = await request(server)
        .delete(`/api/v1/call-lists/${created.id}`)
        .set(authHeaders)
        .expect(200);
