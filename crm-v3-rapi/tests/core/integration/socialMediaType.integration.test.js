 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for SocialMediaType CRUD operations.
...
describe('SocialMediaType Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('POST /api/v1/social-media-types - Create', () => {
    it('should create a new SocialMediaType and persist to database', async () => {
      ...
      const dbRecord = await prisma.socialMediaType.findUnique({
  ...
  describe('GET /api/v1/social-media-types - List', () => {
    it('should return paginated list of SocialMediaTypes', async () => {
      // Create test records
      await createSocialMediaType();
      await createSocialMediaType();
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/social-media-types?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/social-media-types/:id - Get One', () => {
    it('should return a specific SocialMediaType by ID', async () => {
      const created = await createSocialMediaType();

      const response = await request(server)
        .get(`/api/v1/social-media-types/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/social-media-types/:id - Update', () => {
    it('should update an existing SocialMediaType', async () => {
      const created = await createSocialMediaType();
      const updateData = {
      ...
      const response = await request(server)
        .put(`/api/v1/social-media-types/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/social-media-types/:id - Delete', () => {
    it('should delete an existing SocialMediaType', async () => {
      const created = await createSocialMediaType();

      const response = await request(server)
        .delete(`/api/v1/social-media-types/${created.id}`)
        .set(authHeaders)
        .expect(200);
