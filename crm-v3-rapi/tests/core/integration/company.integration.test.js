...
describe('Company Integration Tests', () => {
  ...
  describe('POST /api/v1/companies - Create', () => {
    it('should create a new Company and persist to database', async () => {
      ...
      const dbRecord = await prisma.company.findUnique({
        where: { id: response.body.id },
      });

      expect(dbRecord).not.toBeNull();
      expect(dbRecord.name).toBeDefined();
      expect(dbRecord.betaPartners).toBeDefined();
    });
  ...
  describe('GET /api/v1/companies - List', () => {
    it('should return paginated list of Companys', async () => {
      // Create test records
      await createCompany();
      await createCompany();
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/companies?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/companies/:id - Get One', () => {
    ...
    it('should include all expected fields in response', async () => {
      ...
      // email is optional
      // fax is optional
      // staffUrl is optional
      expect(response.body.createdAt).toBeDefined();
      expect(response.body.updatedAt).toBeDefined();
    });
  });

  describe('PUT /api/v1/companies/:id - Update', () => {
    it('should update an existing Company', async () => {
      ...
      const response = await request(server)
        .put(`/api/v1/companies/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/companies/:id - Delete', () => {
