 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for CallListPipeline CRUD operations.
...
describe('CallListPipeline Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('POST /api/v1/call-list-pipelines - Create', () => {
    it('should create a new CallListPipeline and persist to database', async () => {
      ...
      const dbRecord = await prisma.callListPipeline.findUnique({
  ...
  describe('GET /api/v1/call-list-pipelines - List', () => {
    it('should return paginated list of CallListPipelines', async () => {
      // Create test records
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/call-list-pipelines?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/call-list-pipelines/:id - Get One', () => {
    it('should return a specific CallListPipeline by ID', async () => {
      const created = await createCallListPipeline();

      const response = await request(server)
        .get(`/api/v1/call-list-pipelines/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/call-list-pipelines/:id - Update', () => {
    it('should update an existing CallListPipeline', async () => {
      const created = await createCallListPipeline();
      const updateData = {
      ...
      const response = await request(server)
        .put(`/api/v1/call-list-pipelines/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/call-list-pipelines/:id - Delete', () => {
    it('should delete an existing CallListPipeline', async () => {
      const created = await createCallListPipeline();

      const response = await request(server)
        .delete(`/api/v1/call-list-pipelines/${created.id}`)
        .set(authHeaders)
        .expect(200);
