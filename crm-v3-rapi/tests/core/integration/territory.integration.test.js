 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for Territory CRUD operations.
...
describe('Territory Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('POST /api/v1/territories - Create', () => {
    it('should create a new Territory and persist to database', async () => {
      ...
      const dbRecord = await prisma.territory.findUnique({
  ...
  describe('GET /api/v1/territories - List', () => {
    it('should return paginated list of Territorys', async () => {
      // Create test records
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/territories?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/territories/:id - Get One', () => {
    it('should return a specific Territory by ID', async () => {
      const created = await createTerritory();

      const response = await request(server)
        .get(`/api/v1/territories/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/territories/:id - Update', () => {
    it('should update an existing Territory', async () => {
      ...
      const response = await request(server)
        .put(`/api/v1/territories/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/territories/:id - Delete', () => {
    it('should delete an existing Territory', async () => {
      const created = await createTerritory();

      const response = await request(server)
        .delete(`/api/v1/territories/${created.id}`)
        .set(authHeaders)
        .expect(200);
