 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for Prospect CRUD operations.
...
const {
  buildProspectForApi,
  createProspect,
} = require('#tests/factories/prospect.factory.js');

describe('Prospect Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  afterEach(async () => {
    ...
    await cleanupTestRecords('prospect');
  });
  ...
  describe('POST /api/v1/prospects - Create', () => {
    it('should create a new Prospect and persist to database', async () => {
      const testData = await buildProspectForApi();
      ...
      const dbRecord = await prisma.prospect.findUnique({
        where: { id: response.body.id },
      });

      expect(dbRecord).not.toBeNull();
      expect(dbRecord.qualificationScore).toBeDefined();
      expect(dbRecord.temperature).toBeDefined();
    });
  ...
  describe('GET /api/v1/prospects - List', () => {
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/prospects?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/prospects/:id - Get One', () => {
    it('should return a specific Prospect by ID', async () => {
      const created = await createProspect();

      const response = await request(server)
        .get(`/api/v1/prospects/${created.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should include all expected fields in response', async () => {
      ...
      // disqualificationReason is optional
      // sourceCampaign is optional
      // interestSummary is optional
      expect(response.body.qualificationScore).toBeDefined();
  ...
  describe('PUT /api/v1/prospects/:id - Update', () => {
    it('should update an existing Prospect', async () => {
      ...
      const response = await request(server)
        .put(`/api/v1/prospects/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);

      expect(response.body.id).toBe(created.id);
      expect(response.body.disqualificationReason).toBeDefined();
  ...
  describe('DELETE /api/v1/prospects/:id - Delete', () => {
