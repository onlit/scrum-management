 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for OpportunityProduct CRUD operations.
...
describe('OpportunityProduct Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('POST /api/v1/opportunity-products - Create', () => {
    it('should create a new OpportunityProduct and persist to database', async () => {
      ...
      const dbRecord = await prisma.opportunityProduct.findUnique({
  ...
  describe('GET /api/v1/opportunity-products - List', () => {
    it('should return paginated list of OpportunityProducts', async () => {
      // Create test records
      await createOpportunityProduct();
      await createOpportunityProduct();
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/opportunity-products?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/opportunity-products/:id - Get One', () => {
    it('should return a specific OpportunityProduct by ID', async () => {
      const created = await createOpportunityProduct();

      const response = await request(server)
        .get(`/api/v1/opportunity-products/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/opportunity-products/:id - Update', () => {
    it('should update an existing OpportunityProduct', async () => {
      const created = await createOpportunityProduct();
      const updateData = {
        amount: 42,
      };
      ...
      const response = await request(server)
        .put(`/api/v1/opportunity-products/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/opportunity-products/:id - Delete', () => {
    it('should delete an existing OpportunityProduct', async () => {
      const created = await createOpportunityProduct();

      const response = await request(server)
        .delete(`/api/v1/opportunity-products/${created.id}`)
        .set(authHeaders)
        .expect(200);
