 ...
 * DESCRIPTION:
 * ------------------
 * Integration tests for CustomerEnquiry CRUD operations.
...
describe('CustomerEnquiry Integration Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('POST /api/v1/customer-enquiries - Create', () => {
    it('should create a new CustomerEnquiry and persist to database', async () => {
      ...
      const dbRecord = await prisma.customerEnquiry.findUnique({
        where: { id: response.body.id },
      });

      expect(dbRecord).not.toBeNull();
      expect(dbRecord.firstName).toBeDefined();
      expect(dbRecord.lastName).toBeDefined();
      expect(dbRecord.sourceNotes).toBeDefined();
    });
  ...
  describe('GET /api/v1/customer-enquiries - List', () => {
    it('should return paginated list of CustomerEnquirys', async () => {
      // Create test records
    ...
    it('should support pagination parameters', async () => {
      ...
      const response = await request(server)
        .get('/api/v1/customer-enquiries?page=1&perPage=2')
        .set(authHeaders)
        .expect(200);
  ...
  describe('GET /api/v1/customer-enquiries/:id - Get One', () => {
    it('should return a specific CustomerEnquiry by ID', async () => {
      const created = await createCustomerEnquiry();

      const response = await request(server)
        .get(`/api/v1/customer-enquiries/${created.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/customer-enquiries/:id - Update', () => {
    it('should update an existing CustomerEnquiry', async () => {
      const created = await createCustomerEnquiry();
      const updateData = {
      ...
      const response = await request(server)
        .put(`/api/v1/customer-enquiries/${created.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/customer-enquiries/:id - Delete', () => {
    it('should delete an existing CustomerEnquiry', async () => {
      const created = await createCustomerEnquiry();

      const response = await request(server)
        .delete(`/api/v1/customer-enquiries/${created.id}`)
        .set(authHeaders)
        .expect(200);
