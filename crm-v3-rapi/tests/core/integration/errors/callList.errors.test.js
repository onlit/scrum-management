 ...
 * DESCRIPTION:
 * ------------------
 * Error handling tests for CallList endpoints.
...
const { createCallList } = require('#tests/factories/callList.factory.js');

describe('CallList Error Handling', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  afterEach(async () => {
    await cleanupTestRecords('callList');
  });

  describe('404 Not Found Errors', () => {
    it('GET /api/v1/call-lists/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;

      const response = await request(server)
        .get(`/api/v1/call-lists/${fakeId}`)
        .set(authHeaders)
        .expect(404);
    ...
    it('PUT /api/v1/call-lists/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;

      const response = await request(server)
        .put(`/api/v1/call-lists/${fakeId}`)
        .set(authHeaders)
        .send({
    ...
    it('DELETE /api/v1/call-lists/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;

      const response = await request(server)
        .delete(`/api/v1/call-lists/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('400 Bad Request - Invalid Input', () => {
    it('GET /api/v1/call-lists/:id returns 400 for invalid UUID format', async () => {
      const invalidId = INVALID_UUID;

      const response = await request(server)
        .get(`/api/v1/call-lists/${invalidId}`)
        .set(authHeaders)
        .expect(400);
    ...
    it('POST /api/v1/call-lists returns 400 for missing required fields', async () => {
      const response = await request(server)
    ...
    it('POST /api/v1/call-lists returns 400 when name is missing', async () => {
      const invalidData = {
  ...
  describe('422 Validation Errors', () => {
    it('POST /api/v1/call-lists returns validation error for invalid email format', async () => {
      const invalidData = {
  ...
  describe('409 Conflict Errors', () => {
    it('POST /api/v1/call-lists returns 409 for duplicate unique constraint', async () => {
      ...
      // Create first record
      const firstRecord = await createCallList();
