 ...
 * DESCRIPTION:
 * ------------------
 * Error handling tests for Client endpoints.
...
const { createClient } = require('#tests/factories/client.factory.js');
...
describe('Client Error Handling', () => {
  ...
  describe('404 Not Found Errors', () => {
    it('GET /api/v1/clients/:id returns 404 for non-existent ID', async () => {
    ...
    it('PUT /api/v1/clients/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;
    ...
    it('DELETE /api/v1/clients/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;
  ...
  describe('400 Bad Request - Invalid Input', () => {
    it('GET /api/v1/clients/:id returns 400 for invalid UUID format', async () => {
    ...
    it('POST /api/v1/clients returns 400 for missing required fields', async () => {
      const response = await request(server)
        .post('/api/v1/clients')
        .set(authHeaders)
        .send({}) // Empty body
  ...
  describe('422 Validation Errors', () => {
    it('POST /api/v1/clients returns validation error for invalid email format', async () => {
      const invalidData = {
  ...
  describe('409 Conflict Errors', () => {
    it('POST /api/v1/clients returns 409 for duplicate unique constraint', async () => {
      ...
      // Create first record
      const firstRecord = await createClient();
