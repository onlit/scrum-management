 ...
 * DESCRIPTION:
 * ------------------
 * Error handling tests for Company endpoints.
...
describe('Company Error Handling', () => {
  ...
  afterEach(async () => {
    await cleanupTestRecords('company');
  });

  describe('404 Not Found Errors', () => {
    it('GET /api/v1/companies/:id returns 404 for non-existent ID', async () => {
    ...
    it('PUT /api/v1/companies/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;
    ...
    it('DELETE /api/v1/companies/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;
  ...
  describe('400 Bad Request - Invalid Input', () => {
    it('GET /api/v1/companies/:id returns 400 for invalid UUID format', async () => {
    ...
    it('POST /api/v1/companies returns 400 for missing required fields', async () => {
      const response = await request(server)
    ...
    it('POST /api/v1/companies returns 400 when name is missing', async () => {
      const invalidData = {
  ...
  describe('422 Validation Errors', () => {
    it('POST /api/v1/companies returns validation error for invalid email format', async () => {
      const invalidData = {
    ...
    it('POST /api/v1/companies returns error for invalid URL format', async () => {
      const invalidData = {
        staffUrl: 'not-a-valid-url',
      };
  ...
  describe('409 Conflict Errors', () => {
    it('POST /api/v1/companies returns 409 for duplicate unique constraint', async () => {
      ...
      // Create first record
      const firstRecord = await createCompany();
