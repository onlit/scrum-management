 ...
 * DESCRIPTION:
 * ------------------
 * Error handling tests for Person endpoints.
...
describe('Person Error Handling', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('404 Not Found Errors', () => {
    it('GET /api/v1/people/:id returns 404 for non-existent ID', async () => {
    ...
    it('PUT /api/v1/people/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;
    ...
    it('DELETE /api/v1/people/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;
  ...
  describe('400 Bad Request - Invalid Input', () => {
    it('GET /api/v1/people/:id returns 400 for invalid UUID format', async () => {
    ...
    it('POST /api/v1/people returns 400 for missing required fields', async () => {
      const response = await request(server)
        .post('/api/v1/people')
        .set(authHeaders)
        .send({}) // Empty body
    ...
    it('POST /api/v1/people returns 400 when firstName is missing', async () => {
      const invalidData = {
  ...
  describe('422 Validation Errors', () => {
    it('POST /api/v1/people returns validation error for invalid email format', async () => {
      const invalidData = {
  ...
  describe('409 Conflict Errors', () => {
    it('POST /api/v1/people returns 409 for duplicate unique constraint', async () => {
      ...
      // Create first record
      const firstRecord = await createPerson();
