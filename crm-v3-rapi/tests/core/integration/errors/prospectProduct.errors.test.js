 ...
 * DESCRIPTION:
 * ------------------
 * Error handling tests for ProspectProduct endpoints.
...
describe('ProspectProduct Error Handling', () => {
  let server;
  let prisma;
  let authHeaders;
  ...
  describe('404 Not Found Errors', () => {
    it('GET /api/v1/prospect-products/:id returns 404 for non-existent ID', async () => {
    ...
    it('PUT /api/v1/prospect-products/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;

      const response = await request(server)
        .put(`/api/v1/prospect-products/${fakeId}`)
        .set(authHeaders)
        .send({
          amount: 42,
        })
    ...
    it('DELETE /api/v1/prospect-products/:id returns 404 for non-existent ID', async () => {
      const fakeId = NON_EXISTENT_UUID;

      const response = await request(server)
        .delete(`/api/v1/prospect-products/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('400 Bad Request - Invalid Input', () => {
    it('GET /api/v1/prospect-products/:id returns 400 for invalid UUID format', async () => {
      const invalidId = INVALID_UUID;

      const response = await request(server)
        .get(`/api/v1/prospect-products/${invalidId}`)
        .set(authHeaders)
        .expect(400);
    ...
    it('POST /api/v1/prospect-products returns 400 for missing required fields', async () => {
      const response = await request(server)
  ...
  describe('422 Validation Errors', () => {
    it('POST /api/v1/prospect-products returns validation error for invalid email format', async () => {
      const invalidData = {
  ...
  describe('409 Conflict Errors', () => {
    it('POST /api/v1/prospect-products returns 409 for duplicate unique constraint', async () => {
      ...
      // Create first record
      const firstRecord = await createProspectProduct();
