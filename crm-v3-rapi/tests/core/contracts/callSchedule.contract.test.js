 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CallSchedule API endpoints.
...
} = require('#tests/core/contracts/schemas/callSchedule.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CallSchedule Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  describe('GET /api/v1/call-schedules', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CallScheduleListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/call-schedules', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCallScheduleForApi to ensure required relations exist
      ...
      const { error } = CallScheduleResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/call-schedules/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/call-schedules/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/call-schedules/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCallScheduleForApi to ensure required relations exist
      const updateData = await buildCallScheduleForApi();

      const response = await request(server)
        .put(`/api/v1/call-schedules/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/call-schedules/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCallSchedule();

      const response = await request(server)
        .delete(`/api/v1/call-schedules/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
