 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Relationship API endpoints.
...
} = require('#tests/core/contracts/schemas/relationship.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('Relationship Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createRelationship();
  });
  ...
  describe('GET /api/v1/relationships', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = RelationshipListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/relationships', () => {
    it('should return created record matching response schema', async () => {
      // Use buildRelationshipForApi to ensure required relations exist
      const testData = await buildRelationshipForApi();
      ...
      const { error } = RelationshipResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/relationships/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/relationships/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/relationships/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildRelationshipForApi to ensure required relations exist
      const updateData = await buildRelationshipForApi();

      const response = await request(server)
        .put(`/api/v1/relationships/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/relationships/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createRelationship();

      const response = await request(server)
        .delete(`/api/v1/relationships/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
