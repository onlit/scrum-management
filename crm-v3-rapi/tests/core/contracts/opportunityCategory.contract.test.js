 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for OpportunityCategory API endpoints.
...
} = require('#tests/core/contracts/schemas/opportunityCategory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('OpportunityCategory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createOpportunityCategory();
  });
  ...
  describe('GET /api/v1/opportunity-categories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = OpportunityCategoryListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/opportunity-categories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildOpportunityCategoryForApi to ensure required relations exist
      ...
      const { error } = OpportunityCategoryResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/opportunity-categories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/opportunity-categories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/opportunity-categories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildOpportunityCategoryForApi to ensure required relations exist
      const updateData = await buildOpportunityCategoryForApi();

      const response = await request(server)
        .put(`/api/v1/opportunity-categories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/opportunity-categories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createOpportunityCategory();

      const response = await request(server)
        .delete(`/api/v1/opportunity-categories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
