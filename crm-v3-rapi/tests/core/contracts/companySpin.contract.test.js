 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CompanySpin API endpoints.
...
} = require('#tests/core/contracts/schemas/companySpin.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CompanySpin Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  describe('GET /api/v1/company-spins', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CompanySpinListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/company-spins', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCompanySpinForApi to ensure required relations exist
      ...
      const { error } = CompanySpinResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/company-spins/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/company-spins/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/company-spins/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCompanySpinForApi to ensure required relations exist
      const updateData = await buildCompanySpinForApi();

      const response = await request(server)
        .put(`/api/v1/company-spins/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/company-spins/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCompanySpin();

      const response = await request(server)
        .delete(`/api/v1/company-spins/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
