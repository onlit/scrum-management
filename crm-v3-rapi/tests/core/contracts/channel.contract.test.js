 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Channel API endpoints.
...
const {
  buildChannelForApi,
  createChannel,
} = require('#tests/factories/channel.factory.js');
const {
  ChannelResponseSchema,
  ChannelListResponseSchema,
} = require('#tests/core/contracts/schemas/channel.schema.js');
const {
  NotFoundErrorSchema,
...
describe('Channel Contract Tests', () => {
  ...
  beforeAll(async () => {
    ...
    testRecord = await createChannel();
  });
  ...
  describe('GET /api/v1/channels', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ChannelListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/channels', () => {
    it('should return created record matching response schema', async () => {
      // Use buildChannelForApi to ensure required relations exist
      const testData = await buildChannelForApi();
      ...
      const { error } = ChannelResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/channels/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/channels/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/channels/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildChannelForApi to ensure required relations exist
      const updateData = await buildChannelForApi();

      const response = await request(server)
        .put(`/api/v1/channels/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/channels/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createChannel();

      const response = await request(server)
        .delete(`/api/v1/channels/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
