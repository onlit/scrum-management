 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for PersonRelationship API endpoints.
...
} = require('#tests/core/contracts/schemas/personRelationship.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('PersonRelationship Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createPersonRelationship();
  });
  ...
  describe('GET /api/v1/person-relationships', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = PersonRelationshipListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/person-relationships', () => {
    it('should return created record matching response schema', async () => {
      // Use buildPersonRelationshipForApi to ensure required relations exist
      ...
      const { error } = PersonRelationshipResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/person-relationships/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/person-relationships/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/person-relationships/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/person-relationships/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildPersonRelationshipForApi to ensure required relations exist
      const updateData = await buildPersonRelationshipForApi();

      const response = await request(server)
        .put(`/api/v1/person-relationships/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/person-relationships/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createPersonRelationship();

      const response = await request(server)
        .delete(`/api/v1/person-relationships/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
