 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Opportunity API endpoints.
...
} = require('#tests/core/contracts/schemas/opportunity.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('Opportunity Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createOpportunity();
  });
  ...
  describe('GET /api/v1/opportunities', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = OpportunityListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/opportunities', () => {
    it('should return created record matching response schema', async () => {
      // Use buildOpportunityForApi to ensure required relations exist
      ...
      const { error } = OpportunityResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/opportunities/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/opportunities/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/opportunities/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildOpportunityForApi to ensure required relations exist
      const updateData = await buildOpportunityForApi();

      const response = await request(server)
        .put(`/api/v1/opportunities/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/opportunities/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createOpportunity();

      const response = await request(server)
        .delete(`/api/v1/opportunities/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
