 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Client API endpoints.
...
const {
  buildClientForApi,
  createClient,
} = require('#tests/factories/client.factory.js');
const {
  ClientResponseSchema,
  ClientListResponseSchema,
} = require('#tests/core/contracts/schemas/client.schema.js');
const {
  NotFoundErrorSchema,
...
describe('Client Contract Tests', () => {
  ...
  describe('GET /api/v1/clients', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ClientListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/clients', () => {
    it('should return created record matching response schema', async () => {
      // Use buildClientForApi to ensure required relations exist
      const testData = await buildClientForApi();
      ...
      const { error } = ClientResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/clients/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/clients/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/clients/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildClientForApi to ensure required relations exist
      const updateData = await buildClientForApi();

      const response = await request(server)
        .put(`/api/v1/clients/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/clients/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createClient();

      const response = await request(server)
        .delete(`/api/v1/clients/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
