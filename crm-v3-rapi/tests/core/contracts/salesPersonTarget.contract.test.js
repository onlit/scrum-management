 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for SalesPersonTarget API endpoints.
...
} = require('#tests/core/contracts/schemas/salesPersonTarget.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('SalesPersonTarget Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.salesPersonTarget.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createSalesPersonTarget();
  });
  ...
  describe('GET /api/v1/sales-person-targets', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = SalesPersonTargetListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/sales-person-targets', () => {
    it('should return created record matching response schema', async () => {
      // Use buildSalesPersonTargetForApi to ensure required relations exist
      ...
      const { error } = SalesPersonTargetResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/sales-person-targets/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/sales-person-targets/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/sales-person-targets/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/sales-person-targets/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildSalesPersonTargetForApi to ensure required relations exist
      const updateData = await buildSalesPersonTargetForApi();

      const response = await request(server)
        .put(`/api/v1/sales-person-targets/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/sales-person-targets/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createSalesPersonTarget();

      const response = await request(server)
        .delete(`/api/v1/sales-person-targets/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
