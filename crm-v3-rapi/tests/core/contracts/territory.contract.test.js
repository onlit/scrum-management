 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Territory API endpoints.
...
const {
  ...
  createTerritory,
} = require('#tests/factories/territory.factory.js');
const {
  TerritoryResponseSchema,
  TerritoryListResponseSchema,
} = require('#tests/core/contracts/schemas/territory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('Territory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createTerritory();
  });
  ...
  describe('GET /api/v1/territories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = TerritoryListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/territories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildTerritoryForApi to ensure required relations exist
      const testData = await buildTerritoryForApi();
      ...
      const { error } = TerritoryResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/territories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/territories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/territories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildTerritoryForApi to ensure required relations exist
      const updateData = await buildTerritoryForApi();

      const response = await request(server)
        .put(`/api/v1/territories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/territories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createTerritory();

      const response = await request(server)
        .delete(`/api/v1/territories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
