 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Prospect API endpoints.
...
} = require('#tests/core/contracts/schemas/prospect.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('Prospect Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createProspect();
  });
  ...
  describe('GET /api/v1/prospects', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ProspectListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/prospects', () => {
    it('should return created record matching response schema', async () => {
      // Use buildProspectForApi to ensure required relations exist
      ...
      const { error } = ProspectResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/prospects/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/prospects/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/prospects/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildProspectForApi to ensure required relations exist
      const updateData = await buildProspectForApi();

      const response = await request(server)
        .put(`/api/v1/prospects/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/prospects/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createProspect();

      const response = await request(server)
        .delete(`/api/v1/prospects/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
