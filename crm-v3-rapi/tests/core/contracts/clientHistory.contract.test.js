 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for ClientHistory API endpoints.
...
} = require('#tests/core/contracts/schemas/clientHistory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('ClientHistory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createClientHistory();
  });
  ...
  describe('GET /api/v1/client-histories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ClientHistoryListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/client-histories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildClientHistoryForApi to ensure required relations exist
      ...
      const { error } = ClientHistoryResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/client-histories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/client-histories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/client-histories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildClientHistoryForApi to ensure required relations exist
      const updateData = await buildClientHistoryForApi();

      const response = await request(server)
        .put(`/api/v1/client-histories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/client-histories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createClientHistory();

      const response = await request(server)
        .delete(`/api/v1/client-histories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
