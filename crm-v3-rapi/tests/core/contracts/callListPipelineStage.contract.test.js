 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CallListPipelineStage API endpoints.
...
} = require('#tests/core/contracts/schemas/callListPipelineStage.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CallListPipelineStage Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.callListPipelineStage.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createCallListPipelineStage();
  });
  ...
  describe('GET /api/v1/call-list-pipeline-stages', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CallListPipelineStageListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/call-list-pipeline-stages', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCallListPipelineStageForApi to ensure required relations exist
      ...
      const { error } = CallListPipelineStageResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/call-list-pipeline-stages/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/call-list-pipeline-stages/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/call-list-pipeline-stages/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCallListPipelineStageForApi to ensure required relations exist
      const updateData = await buildCallListPipelineStageForApi();

      const response = await request(server)
        .put(`/api/v1/call-list-pipeline-stages/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/call-list-pipeline-stages/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCallListPipelineStage();

      const response = await request(server)
        .delete(`/api/v1/call-list-pipeline-stages/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
