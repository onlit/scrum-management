 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CompanyHistory API endpoints.
...
} = require('#tests/core/contracts/schemas/companyHistory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CompanyHistory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createCompanyHistory();
  });
  ...
  describe('GET /api/v1/company-histories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CompanyHistoryListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/company-histories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCompanyHistoryForApi to ensure required relations exist
      ...
      const { error } = CompanyHistoryResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/company-histories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/company-histories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/company-histories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCompanyHistoryForApi to ensure required relations exist
      const updateData = await buildCompanyHistoryForApi();

      const response = await request(server)
        .put(`/api/v1/company-histories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/company-histories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCompanyHistory();

      const response = await request(server)
        .delete(`/api/v1/company-histories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
