 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for PersonHistory API endpoints.
...
} = require('#tests/core/contracts/schemas/personHistory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('PersonHistory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createPersonHistory();
  });
  ...
  describe('GET /api/v1/person-histories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = PersonHistoryListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/person-histories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildPersonHistoryForApi to ensure required relations exist
      ...
      const { error } = PersonHistoryResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/person-histories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/person-histories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/person-histories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildPersonHistoryForApi to ensure required relations exist
      const updateData = await buildPersonHistoryForApi();

      const response = await request(server)
        .put(`/api/v1/person-histories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/person-histories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createPersonHistory();

      const response = await request(server)
        .delete(`/api/v1/person-histories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
