 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for DataNeeded API endpoints.
...
} = require('#tests/core/contracts/schemas/dataNeeded.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('DataNeeded Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  describe('GET /api/v1/data-neededs', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = DataNeededListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/data-neededs', () => {
    it('should return created record matching response schema', async () => {
      // Use buildDataNeededForApi to ensure required relations exist
      ...
      const { error } = DataNeededResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/data-neededs/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/data-neededs/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/data-neededs/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildDataNeededForApi to ensure required relations exist
      const updateData = await buildDataNeededForApi();

      const response = await request(server)
        .put(`/api/v1/data-neededs/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/data-neededs/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createDataNeeded();

      const response = await request(server)
        .delete(`/api/v1/data-neededs/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
