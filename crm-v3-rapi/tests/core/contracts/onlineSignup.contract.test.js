 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for OnlineSignup API endpoints.
...
} = require('#tests/core/contracts/schemas/onlineSignup.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('OnlineSignup Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  describe('GET /api/v1/online-signups', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = OnlineSignupListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/online-signups', () => {
    it('should return created record matching response schema', async () => {
      // Use buildOnlineSignupForApi to ensure required relations exist
      ...
      const { error } = OnlineSignupResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/online-signups/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/online-signups/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/online-signups/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildOnlineSignupForApi to ensure required relations exist
      const updateData = await buildOnlineSignupForApi();

      const response = await request(server)
        .put(`/api/v1/online-signups/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/online-signups/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createOnlineSignup();

      const response = await request(server)
        .delete(`/api/v1/online-signups/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
