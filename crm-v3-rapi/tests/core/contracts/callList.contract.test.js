 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CallList API endpoints.
...
} = require('#tests/core/contracts/schemas/callList.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CallList Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createCallList();
  });
  ...
  describe('GET /api/v1/call-lists', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CallListListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/call-lists', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCallListForApi to ensure required relations exist
      ...
      const { error } = CallListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/call-lists/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/call-lists/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/call-lists/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCallListForApi to ensure required relations exist
      const updateData = await buildCallListForApi();

      const response = await request(server)
        .put(`/api/v1/call-lists/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/call-lists/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCallList();

      const response = await request(server)
        .delete(`/api/v1/call-lists/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
