 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CallHistory API endpoints.
...
} = require('#tests/core/contracts/schemas/callHistory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CallHistory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createCallHistory();
  });
  ...
  describe('GET /api/v1/call-histories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CallHistoryListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/call-histories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCallHistoryForApi to ensure required relations exist
      ...
      const { error } = CallHistoryResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/call-histories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/call-histories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/call-histories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCallHistoryForApi to ensure required relations exist
      const updateData = await buildCallHistoryForApi();

      const response = await request(server)
        .put(`/api/v1/call-histories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/call-histories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCallHistory();

      const response = await request(server)
        .delete(`/api/v1/call-histories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
