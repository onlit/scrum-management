 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CompanyContact API endpoints.
...
} = require('#tests/core/contracts/schemas/companyContact.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CompanyContact Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createCompanyContact();
  });
  ...
  describe('GET /api/v1/company-contacts', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CompanyContactListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/company-contacts', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCompanyContactForApi to ensure required relations exist
      ...
      const { error } = CompanyContactResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/company-contacts/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/company-contacts/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/company-contacts/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/company-contacts/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCompanyContactForApi to ensure required relations exist
      const updateData = await buildCompanyContactForApi();

      const response = await request(server)
        .put(`/api/v1/company-contacts/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/company-contacts/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCompanyContact();

      const response = await request(server)
        .delete(`/api/v1/company-contacts/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
