 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for OpportunityHistory API endpoints.
...
} = require('#tests/core/contracts/schemas/opportunityHistory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('OpportunityHistory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createOpportunityHistory();
  });
  ...
  describe('GET /api/v1/opportunity-histories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = OpportunityHistoryListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/opportunity-histories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildOpportunityHistoryForApi to ensure required relations exist
      ...
      const { error } = OpportunityHistoryResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/opportunity-histories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/opportunity-histories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/opportunity-histories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildOpportunityHistoryForApi to ensure required relations exist
      const updateData = await buildOpportunityHistoryForApi();

      const response = await request(server)
        .put(`/api/v1/opportunity-histories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/opportunity-histories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createOpportunityHistory();

      const response = await request(server)
        .delete(`/api/v1/opportunity-histories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
