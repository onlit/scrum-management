 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for ProspectCategory API endpoints.
...
} = require('#tests/core/contracts/schemas/prospectCategory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('ProspectCategory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.prospectCategory.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createProspectCategory();
  });

  afterAll(async () => {
    await cleanupTestRecords('prospectCategory');
  });

  describe('GET /api/v1/prospect-categories', () => {
    it('should return response matching list schema', async () => {
      const response = await request(server)
        .get('/api/v1/prospect-categories')
        .set(authHeaders)
        .expect(200);

      const { error } = ProspectCategoryListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/prospect-categories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildProspectCategoryForApi to ensure required relations exist
      const testData = await buildProspectCategoryForApi();

      const response = await request(server)
        .post('/api/v1/prospect-categories')
        .set(authHeaders)
        .send(testData)
        .expect(201);

      const { error } = ProspectCategoryResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/prospect-categories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/prospect-categories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/prospect-categories/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/prospect-categories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildProspectCategoryForApi to ensure required relations exist
      const updateData = await buildProspectCategoryForApi();

      const response = await request(server)
        .put(`/api/v1/prospect-categories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/prospect-categories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createProspectCategory();

      const response = await request(server)
        .delete(`/api/v1/prospect-categories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
