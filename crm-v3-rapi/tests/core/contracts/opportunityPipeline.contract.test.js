 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for OpportunityPipeline API endpoints.
...
} = require('#tests/core/contracts/schemas/opportunityPipeline.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('OpportunityPipeline Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.opportunityPipeline.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createOpportunityPipeline();
  });
  ...
  describe('GET /api/v1/opportunity-pipelines', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = OpportunityPipelineListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/opportunity-pipelines', () => {
    it('should return created record matching response schema', async () => {
      // Use buildOpportunityPipelineForApi to ensure required relations exist
      ...
      const { error } = OpportunityPipelineResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/opportunity-pipelines/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/opportunity-pipelines/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/opportunity-pipelines/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/opportunity-pipelines/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildOpportunityPipelineForApi to ensure required relations exist
      const updateData = await buildOpportunityPipelineForApi();

      const response = await request(server)
        .put(`/api/v1/opportunity-pipelines/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/opportunity-pipelines/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createOpportunityPipeline();

      const response = await request(server)
        .delete(`/api/v1/opportunity-pipelines/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
