 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for ProspectProduct API endpoints.
...
} = require('#tests/core/contracts/schemas/prospectProduct.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('ProspectProduct Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.prospectProduct.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createProspectProduct();
  });
  ...
  describe('GET /api/v1/prospect-products', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ProspectProductListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/prospect-products', () => {
    it('should return created record matching response schema', async () => {
      // Use buildProspectProductForApi to ensure required relations exist
      ...
      const { error } = ProspectProductResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/prospect-products/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/prospect-products/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/prospect-products/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/prospect-products/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildProspectProductForApi to ensure required relations exist
      const updateData = await buildProspectProductForApi();

      const response = await request(server)
        .put(`/api/v1/prospect-products/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/prospect-products/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createProspectProduct();

      const response = await request(server)
        .delete(`/api/v1/prospect-products/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
