 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CompanySocialMedia API endpoints.
...
} = require('#tests/core/contracts/schemas/companySocialMedia.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CompanySocialMedia Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createCompanySocialMedia();
  });
  ...
  describe('GET /api/v1/company-social-medias', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CompanySocialMediaListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/company-social-medias', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCompanySocialMediaForApi to ensure required relations exist
      ...
      const { error } = CompanySocialMediaResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/company-social-medias/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/company-social-medias/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/company-social-medias/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCompanySocialMediaForApi to ensure required relations exist
      const updateData = await buildCompanySocialMediaForApi();

      const response = await request(server)
        .put(`/api/v1/company-social-medias/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/company-social-medias/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCompanySocialMedia();

      const response = await request(server)
        .delete(`/api/v1/company-social-medias/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
