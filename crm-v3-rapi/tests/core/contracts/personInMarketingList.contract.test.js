 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for PersonInMarketingList API endpoints.
...
const {
  PersonInMarketingListResponseSchema,
  PersonInMarketingListListResponseSchema,
} = require('#tests/core/contracts/schemas/personInMarketingList.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('PersonInMarketingList Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.personInMarketingList.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createPersonInMarketingList();
  });
  ...
  describe('GET /api/v1/person-in-marketing-lists', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = PersonInMarketingListListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/person-in-marketing-lists', () => {
    it('should return created record matching response schema', async () => {
      // Use buildPersonInMarketingListForApi to ensure required relations exist
      ...
      const { error } = PersonInMarketingListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/person-in-marketing-lists/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/person-in-marketing-lists/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/person-in-marketing-lists/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/person-in-marketing-lists/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildPersonInMarketingListForApi to ensure required relations exist
      const updateData = await buildPersonInMarketingListForApi();

      const response = await request(server)
        .put(`/api/v1/person-in-marketing-lists/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/person-in-marketing-lists/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createPersonInMarketingList();

      const response = await request(server)
        .delete(`/api/v1/person-in-marketing-lists/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
