 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for ProspectPipeline API endpoints.
...
const {
  ProspectPipelineResponseSchema,
  ProspectPipelineListResponseSchema,
} = require('#tests/core/contracts/schemas/prospectPipeline.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('ProspectPipeline Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.prospectPipeline.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createProspectPipeline();
  });
  ...
  describe('GET /api/v1/prospect-pipelines', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ProspectPipelineListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/prospect-pipelines', () => {
    it('should return created record matching response schema', async () => {
      // Use buildProspectPipelineForApi to ensure required relations exist
      ...
      const { error } = ProspectPipelineResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/prospect-pipelines/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/prospect-pipelines/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/prospect-pipelines/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/prospect-pipelines/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildProspectPipelineForApi to ensure required relations exist
      const updateData = await buildProspectPipelineForApi();

      const response = await request(server)
        .put(`/api/v1/prospect-pipelines/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/prospect-pipelines/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createProspectPipeline();

      const response = await request(server)
        .delete(`/api/v1/prospect-pipelines/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
