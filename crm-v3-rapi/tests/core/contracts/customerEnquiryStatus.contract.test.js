 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CustomerEnquiryStatus API endpoints.
...
} = require('#tests/core/contracts/schemas/customerEnquiryStatus.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CustomerEnquiryStatus Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.customerEnquiryStatus.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createCustomerEnquiryStatus();
  });
  ...
  describe('GET /api/v1/customer-enquiry-statuses', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CustomerEnquiryStatusListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/customer-enquiry-statuses', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCustomerEnquiryStatusForApi to ensure required relations exist
      ...
      const { error } = CustomerEnquiryStatusResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/customer-enquiry-statuses/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/customer-enquiry-statuses/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/customer-enquiry-statuses/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/customer-enquiry-statuses/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCustomerEnquiryStatusForApi to ensure required relations exist
      const updateData = await buildCustomerEnquiryStatusForApi();

      const response = await request(server)
        .put(`/api/v1/customer-enquiry-statuses/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/customer-enquiry-statuses/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCustomerEnquiryStatus();

      const response = await request(server)
        .delete(`/api/v1/customer-enquiry-statuses/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
