 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Company API endpoints.
...
const {
  buildCompanyForApi,
  createCompany,
} = require('#tests/factories/company.factory.js');
const {
  CompanyResponseSchema,
  CompanyListResponseSchema,
} = require('#tests/core/contracts/schemas/company.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('Company Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createCompany();
  });
  ...
  describe('GET /api/v1/companies', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CompanyListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/companies', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCompanyForApi to ensure required relations exist
      const testData = await buildCompanyForApi();
      ...
      const { error } = CompanyResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/companies/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/companies/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/companies/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCompanyForApi to ensure required relations exist
      const updateData = await buildCompanyForApi();

      const response = await request(server)
        .put(`/api/v1/companies/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/companies/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCompany();

      const response = await request(server)
        .delete(`/api/v1/companies/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
