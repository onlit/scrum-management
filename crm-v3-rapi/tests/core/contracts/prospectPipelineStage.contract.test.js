 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for ProspectPipelineStage API endpoints.
...
} = require('#tests/core/contracts/schemas/prospectPipelineStage.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('ProspectPipelineStage Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.prospectPipelineStage.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createProspectPipelineStage();
  });
  ...
  describe('GET /api/v1/prospect-pipeline-stages', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ProspectPipelineStageListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/prospect-pipeline-stages', () => {
    it('should return created record matching response schema', async () => {
      // Use buildProspectPipelineStageForApi to ensure required relations exist
      ...
      const { error } = ProspectPipelineStageResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/prospect-pipeline-stages/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/prospect-pipeline-stages/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/prospect-pipeline-stages/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/prospect-pipeline-stages/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildProspectPipelineStageForApi to ensure required relations exist
      const updateData = await buildProspectPipelineStageForApi();

      const response = await request(server)
        .put(`/api/v1/prospect-pipeline-stages/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/prospect-pipeline-stages/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createProspectPipelineStage();

      const response = await request(server)
        .delete(`/api/v1/prospect-pipeline-stages/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
