 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for ActionPlan API endpoints.
...
} = require('#tests/core/contracts/schemas/actionPlan.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('ActionPlan Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createActionPlan();
  });
  ...
  describe('GET /api/v1/action-plans', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ActionPlanListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/action-plans', () => {
    it('should return created record matching response schema', async () => {
      // Use buildActionPlanForApi to ensure required relations exist
      ...
      const { error } = ActionPlanResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/action-plans/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/action-plans/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/action-plans/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildActionPlanForApi to ensure required relations exist
      const updateData = await buildActionPlanForApi();

      const response = await request(server)
        .put(`/api/v1/action-plans/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/action-plans/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createActionPlan();

      const response = await request(server)
        .delete(`/api/v1/action-plans/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
