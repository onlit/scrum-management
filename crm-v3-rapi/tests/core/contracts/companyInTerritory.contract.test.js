 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for CompanyInTerritory API endpoints.
...
} = require('#tests/core/contracts/schemas/companyInTerritory.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('CompanyInTerritory Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.companyInTerritory.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createCompanyInTerritory();
  });
  ...
  describe('GET /api/v1/company-in-territories', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = CompanyInTerritoryListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/company-in-territories', () => {
    it('should return created record matching response schema', async () => {
      // Use buildCompanyInTerritoryForApi to ensure required relations exist
      ...
      const { error } = CompanyInTerritoryResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/company-in-territories/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/company-in-territories/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/company-in-territories/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/company-in-territories/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildCompanyInTerritoryForApi to ensure required relations exist
      const updateData = await buildCompanyInTerritoryForApi();

      const response = await request(server)
        .put(`/api/v1/company-in-territories/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/company-in-territories/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createCompanyInTerritory();

      const response = await request(server)
        .delete(`/api/v1/company-in-territories/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
