 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for MarketingList API endpoints.
...
} = require('#tests/core/contracts/schemas/marketingList.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('MarketingList Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    testRecord = await createMarketingList();
  });
  ...
  describe('GET /api/v1/marketing-lists', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = MarketingListListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/marketing-lists', () => {
    it('should return created record matching response schema', async () => {
      // Use buildMarketingListForApi to ensure required relations exist
      ...
      const { error } = MarketingListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/marketing-lists/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/marketing-lists/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/marketing-lists/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/marketing-lists/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildMarketingListForApi to ensure required relations exist
      const updateData = await buildMarketingListForApi();

      const response = await request(server)
        .put(`/api/v1/marketing-lists/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/marketing-lists/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createMarketingList();

      const response = await request(server)
        .delete(`/api/v1/marketing-lists/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
