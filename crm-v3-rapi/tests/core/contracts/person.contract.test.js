 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for Person API endpoints.
...
} = require('#tests/core/contracts/schemas/person.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('Person Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  describe('GET /api/v1/people', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = PersonListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/people', () => {
    it('should return created record matching response schema', async () => {
      // Use buildPersonForApi to ensure required relations exist
      ...
      const { error } = PersonResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/people/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/people/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/people/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildPersonForApi to ensure required relations exist
      const updateData = await buildPersonForApi();

      const response = await request(server)
        .put(`/api/v1/people/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/people/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createPerson();

      const response = await request(server)
        .delete(`/api/v1/people/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
