 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for OpportunityInfluencer API endpoints.
...
} = require('#tests/core/contracts/schemas/opportunityInfluencer.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('OpportunityInfluencer Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.opportunityInfluencer.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createOpportunityInfluencer();
  });
  ...
  describe('GET /api/v1/opportunity-influencers', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = OpportunityInfluencerListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/opportunity-influencers', () => {
    it('should return created record matching response schema', async () => {
      // Use buildOpportunityInfluencerForApi to ensure required relations exist
      ...
      const { error } = OpportunityInfluencerResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/opportunity-influencers/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/opportunity-influencers/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      const response = await request(server)
        .get(`/api/v1/opportunity-influencers/${fakeId}`)
        .set(authHeaders)
        .expect(404);
  ...
  describe('PUT /api/v1/opportunity-influencers/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildOpportunityInfluencerForApi to ensure required relations exist
      const updateData = await buildOpportunityInfluencerForApi();

      const response = await request(server)
        .put(`/api/v1/opportunity-influencers/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/opportunity-influencers/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createOpportunityInfluencer();

      const response = await request(server)
        .delete(`/api/v1/opportunity-influencers/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
