 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for PersonSocialMedia API endpoints.
...
} = require('#tests/core/contracts/schemas/personSocialMedia.schema.js');
const {
  NotFoundErrorSchema,
  DeleteResponseSchema,
} = require('#tests/core/contracts/schemas/common.schema.js');

describe('PersonSocialMedia Contract Tests', () => {
  let server;
  let prisma;
  let authHeaders;
  let testRecord;
  ...
  beforeAll(async () => {
    ...
    await prisma.personSocialMedia.deleteMany({});

    // Create a test record for GET/PUT/DELETE tests to ensure test independence
    testRecord = await createPersonSocialMedia();
  });
  ...
  describe('GET /api/v1/person-social-medias', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = PersonSocialMediaListResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('POST /api/v1/person-social-medias', () => {
    it('should return created record matching response schema', async () => {
      // Use buildPersonSocialMediaForApi to ensure required relations exist
      ...
      const { error } = PersonSocialMediaResponseSchema.validate(
        response.body,
        {
          abortEarly: false,
        },
  ...
  describe('GET /api/v1/person-social-medias/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/person-social-medias/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
  ...
  describe('PUT /api/v1/person-social-medias/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildPersonSocialMediaForApi to ensure required relations exist
      const updateData = await buildPersonSocialMediaForApi();

      const response = await request(server)
        .put(`/api/v1/person-social-medias/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/person-social-medias/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createPersonSocialMedia();

      const response = await request(server)
        .delete(`/api/v1/person-social-medias/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
