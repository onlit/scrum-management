...
class InterceptorRegistry {
   ...
   * Merges global interceptors, model interceptor, and defaults.
   *
  ...
  resolve(modelName) {
    ...
    const modelInterceptor = this.interceptors.get(modelName) || {};
    const defaults = createNoOpInterceptor();

    // Build composed interceptor for each hook
    const composed = {};

    for (const hook of LIFECYCLE_HOOKS) {
      composed[hook] = this._composeHook(hook, modelInterceptor, defaults);
    }

    this.resolveCache.set(modelName, composed);
    return composed;
  }
