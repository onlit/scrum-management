...
const { createError, isTestEnv } = require('./generalUtils.js');
const {
  ERROR_TYPES,
  ERROR_SEVERITY,
  ERROR_MESSAGES,
...
/**
 * Check if an error is a missing table error from Prisma
 * @param {Error} error - The error to check
 * @returns {boolean} True if the error indicates a missing table
 */
const isMissingTableError = (error) =>
  error?.code === 'P2021' || /does not exist/i.test(error?.message || '');

/**
 * Log a warning when a table is missing (only in test environment)
 * @param {string} model - The model name that was being accessed
 * @param {Error} error - The error that occurred
 */
const logMissingTableWarning = (model, error) => {
  if (!isTestEnv()) return;
  const message = error?.message ? error.message.split('\n')[0] : 'Unknown error';
  console.warn(
    `[PRISMA_TEST_SKIP] Skipping updateMany for missing table ${model}: ${message}`
  );
};
...
/**
 * Handles filter validation errors from parseFilters middleware
 * @param {Array} filterErrors - Array of filter error objects
 * @param {string} context - Context where validation failed
 * @returns {Error} Standardized error
 */
function handleFilterValidationError(filterErrors, context = 'filter_parsing') {
  const error = createStandardError(
    ERROR_TYPES.BAD_REQUEST,
  ...
  error.filterErrors = filterErrors;
  return error;
}
