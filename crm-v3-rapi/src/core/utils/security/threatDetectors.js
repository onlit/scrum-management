...
const { logWithTrace, getTraceId } = require('#utils/traceUtils.js');
...
async function detectSQLInjection(input) {
  ...
  const sqlPatterns = [
    ...
    },
    {
      name: 'sql_keywords',
      pattern:
        /\b(?:select|insert|update|delete|drop|alter|create|truncate)\s+/i,
  ...
  for (const { name, pattern, description } of sqlPatterns) {
    if (await safeRegexTest(pattern, input, 50)) {
      // 50ms timeout for ReDoS protection
...
function extractStringsFromObject(obj, maxDepth = 3) {
  ...
  function traverse(item, depth) {
    ...
    if (typeof item === 'string') {
      ...
      for (let i = 0; i < Math.min(item.length, 50); i++) {
        // Limit array size
...
async function analyzeThreats(requestData) {
  ...
  try {
    ...
    if (query) {
      const queryStrings =
        typeof query === 'string' ? [query] : extractStringsFromObject(query);
    ...
    if (body) {
      const bodyStrings =
        typeof body === 'string' ? [body] : extractStringsFromObject(body);
    ...
    if (req) {
      logWithTrace('Threat analysis error in analyzeThreats', req, {
        error: error.message,
