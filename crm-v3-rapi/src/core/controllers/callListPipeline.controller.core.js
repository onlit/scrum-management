...
const interceptor = getRegistry().resolve('CallListPipeline');
...
const MODEL_NAME_LITERAL = 'CallListPipeline';
...
async function bulkUpdateCallListPipelineVisibility(req, res) {
  const { user, body } = req;
  const context = createOperationContext(req, 'CallListPipeline', 'update');

  logOperationStart('bulkUpdateCallListPipelineVisibility', req, {
    user: user?.id,
  ...
  try {
    ...
    if (!clientId) {
      const error = createErrorWithTrace(
        ...
        req,
        {
          ...
          context: 'bulk_update_call_list_pipeline_visibility_client_guard',
        },
      );
      throw error;
    }
    ...
    if (
      ...
      const error = createErrorWithTrace(
        ...
        req,
        {
          ...
          context: 'bulk_update_call_list_pipeline_visibility_permission_check',
        },
      );
      throw error;
    }
    ...
    logDatabaseStart('bulk_update_call_list_pipeline_visibility', req, {
    ...
    logDatabaseSuccess('bulk_update_call_list_pipeline_visibility', req, {
      updatedCount: result.count,
    });

    // Log operation success
    logOperationSuccess('bulkUpdateCallListPipelineVisibility', req, {
      updatedCount: result.count,
    });
    ...
    const handled = await handleControllerError(error, {
      ...
      res,
      context,
      operationName: 'bulk_update_call_list_pipeline_visibility',
    });
    if (handled) return;
  }
}
...
async function getAllCallListPipeline(req, res) {
  ...
  const queryBuilder = createQueryBuilder('CallListPipeline');
  const context = createOperationContext(req, 'CallListPipeline', 'list', {
    queryBuilder,
  });
  ...
  try {
    ...
    const handled = await handleControllerError(error, {
      ...
      interceptor,
      context,
      operationName: 'get_all_call_list_pipeline',
    });
    if (handled) return;
  }
}
...
async function createCallListPipeline(req, res) {
  const { user, body } = req;
  const context = createOperationContext(req, 'CallListPipeline', 'create');
  ...
  try {
    ...
    let schema = callListPipelineCreate;
    ...
    const createdWithDisplay = createdDv
      ? { ...newCallListPipelineWithDetails, [DISPLAY_VALUE_PROP]: createdDv }
      : newCallListPipelineWithDetails;
    ...
    const handled = await handleControllerError(error, {
      ...
      interceptor,
      context,
      operationName: 'create_call_list_pipeline',
    });
    if (handled) return;
  }
}
...
async function getCallListPipeline(req, res) {
  ...
  const context = createOperationContext(req, 'CallListPipeline', 'read');
  ...
  try {
    assertValidUuidParam(params?.id, req, 'get_call_list_pipeline_param');
    ...
    const foundWithDisplay = foundDv
      ? { ...foundCallListPipelineWithDetails, [DISPLAY_VALUE_PROP]: foundDv }
      : foundCallListPipelineWithDetails;
    ...
    const handled = await handleControllerError(error, {
      ...
      interceptor,
      context,
      operationName: 'get_call_list_pipeline',
    });
    if (handled) return;
  }
}
...
async function updateCallListPipeline(req, res) {
  ...
  try {
    assertValidUuidParam(params?.id, req, 'update_call_list_pipeline_param');
    ...
    // Lifecycle: extendSchema - allow custom schema rules
    let schema = callListPipelineUpdate;
    schema = interceptor.extendSchema(schema, context);
    ...
    const updatedDv = computeDisplayValue(
      updatedCallListPipeline,
      MODEL_NAME_LITERAL,
      displayOptions,
    );
    const updatedWithDisplay = updatedDv
      ? { ...updatedCallListPipeline, [DISPLAY_VALUE_PROP]: updatedDv }
      : updatedCallListPipeline;
    ...
    const handled = await handleControllerError(error, {
      ...
      interceptor,
      context,
      operationName: 'update_call_list_pipeline',
    });
    if (handled) return;
  }
}
...
async function deleteCallListPipeline(req, res) {
  const { params, user } = req;
  const context = createOperationContext(req, 'CallListPipeline', 'delete');
  ...
  try {
    assertValidUuidParam(params?.id, req, 'delete_call_list_pipeline_param');
    ...
    const handled = await handleControllerError(error, {
      ...
      interceptor,
      context,
      operationName: 'delete_call_list_pipeline',
    });
    if (handled) return;
  }
}
