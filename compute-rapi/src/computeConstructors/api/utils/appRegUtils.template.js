...
/**
 * Get the URL slug for a model.
 * Uses pre-computed slug from MODEL_SLUGS if available, otherwise falls back to computed slug.
 * @param {string} modelName - Model name (PascalCase)
 * @returns {string} URL slug (kebab-case)
 */
function getModelSlug(modelName) {
  return MODEL_SLUGS[modelName] || convertModelNameToSlug(modelName);
}
...
function buildCapabilities(basePath) {
  ...
  const baseKeyMap = { GET: 'list', POST: 'create' };
  if (baseEntry?.methods) {
    for (const method of Object.keys(baseEntry.methods)) {
      const key = baseKeyMap[method] || method.toLowerCase();
      capabilities.push({
        kind: 'restRoute',
        key,
        spec: {
...
function buildSystemV2Payload() {
  ...
  return {
    app: {
      ...
      tags: null,
      sandboxDomainUrl: 'https://sandbox.{{SLUG}}.pullstream.com',
      stagingDomainUrl: 'https://{{SLUG}}.staging.pullstream.com',
      productionDomainUrl: 'https://{{SLUG}}.pullstream.com',
      backendEnvName: null,
    ...
    const payload = {
      ...
      models: Prisma.dmmf.datamodel.models.map(
        ({ name: modelName, fields }) => ({
          ...
          label: modelName,
          basePath: getModelSlug(modelName),
