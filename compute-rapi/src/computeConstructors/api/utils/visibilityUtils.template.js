...
function parseAndAssignVisibilityAttributes({
  ...
  const bodyOrObj = _.mapKeys(body, (value, key) => toCamelCase(key));
...
function buildCreateRecordPayload(
  ...
  for (const fieldName of Object.keys(sanitizedValues)) {
    if (fieldName.endsWith('Id') && relations?.includes(fieldName)) {
      ...
      const relatedModelName = toCamelCase(fieldName.slice(0, -2));
      ...
      recordPayload[toCamelCase(fieldName)] = sanitizedValues[fieldName];
    }
  }
...
function getVisibilityFilters(user) {
  const rawClient = user?.client?.id;
  const rawUserId = user?.id;
  const client =
    rawClient && validator.isUUID(String(rawClient)) ? String(rawClient) : null;
  const userId =
    rawUserId && validator.isUUID(String(rawUserId)) ? String(rawUserId) : null;
  ...
  // For authenticated internal requests (browser users on internal network), use standard
  // visibility logic instead of the restricted server-to-server logic.
  if (user?.internalRequest && !user?.isAuthenticated) {
    if (client) {
      return { AND: [{ client }] };
    }
  ...
  if (user.isAuthenticated) {
    ...
    const authenticatedUserConditions = [{ everyoneCanSeeIt: true }];

    if (client && userId) {
      authenticatedUserConditions.push({ client, createdBy: userId });
    }
    ...
    if (userId) {
      authenticatedUserConditions.push({
        onlyTheseUsersCanSeeIt: {
          array_contains: [`${userId}|${user.email}`],
        },
      });
    }
