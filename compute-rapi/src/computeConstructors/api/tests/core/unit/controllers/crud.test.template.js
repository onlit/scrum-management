...
// Import the createModelName function from the controller to be tested
const {
  createModelName,
  getAllModelName,
  getModelName,
...
// Mock the Prisma client's modelName model to prevent actual database operations during testing
...
jest.mock('lodash', () => ({
  ...
  isPlainObject: jest.fn().mockImplementation((obj) => obj !== null && typeof obj === 'object' && !Array.isArray(obj)),
...
jest.mock('#domain/interceptors/interceptor.registry.js', () => ({
  getRegistry: jest.fn(() => ({
    resolve: jest.fn(() => ({
      ...
      beforeValidate: jest.fn().mockImplementation((data) => ({ halt: false, data })),
      afterValidate: jest.fn().mockImplementation((data) => ({ halt: false, data })),
      ...
      beforeCreate: jest.fn().mockImplementation((data) => ({ halt: false, data })),
      afterCreate: jest.fn().mockImplementation((data) => ({ data })),
      beforeUpdate: jest.fn().mockImplementation((data) => ({ halt: false, data })),
...
jest.mock('#utils/apiUtils.js', () => ({
  getDetailsFromAPI: jest.fn().mockImplementation(({ results }) => Promise.resolve(results)),
}));
...
// Define a test suite for the createModelName functionality
describe('createModelName', () => {
  ...
  // Test case: successfully creating a modelName
  it('should successfully create a modelName and return status 201 with the created object', async () => {
    // Define mock data for the modelName to be created
    const mockModelNameData = {
      // @gen:MOCK_CREATE_DATA
    };
    ...
    const req = {
      ...
      body: mockModelNameData, // Body contains the mock modelName data
    };
    const res = {
    ...
    // Mock the Prisma create method to return the mock modelName data
    prisma.modelName.create.mockResolvedValue(mockModelNameData);

    // Execute the createModelName function with the mocked request and response
    await createModelName(req, res, next);

    // Assert that the modelName was created successfully
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockModelNameData),
    ); // Check if the created modelName data was returned in the response
  ...
  it('throws an error if validation fails', async () => {
    // Define invalid modelName data to simulate a validation failure
    const invalidModelNameData = {
    ...
    const req = {
      ...
      body: invalidModelNameData,
    };
    const res = {}; // Mock response object not used in this test
    const next = jest.fn(); // Mock the next function to capture errors

    // Execute the createModelName function and expect it to throw an error due to validation failure
    await expect(
      createModelName(req, res, next),
    ).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.modelName.create.mockRejectedValue(new Error('Database error'));
    ...
    // Execute the createModelName function and expect it to throw an error due to database operation failure
    await expect(
      createModelName(req, res, next),
...
// Defines a test suite for the `getAllModelName` function.
describe('getAllModelName', () => {
  ...
  // Defines a test case that verifies the successful retrieval of modelNames.
  it('successfully retrieves modelNames', async () => {
    ...
    // when it successfully fetches modelNames from the database.
      ...
      results: [
        {
          ...
          name: 'ModelName One',
          description: 'Description for ModelName One',
          tags: 'tag1, tag2',
        },
      ],
    };
    ...
    // Invoke the `getAllModelName` function with the mocked request and response objects.
    await getAllModelName(req, res, next);
...
// Describe block defines a test suite for the `getModelName` function.
describe('getModelName', () => {
  ...
  // First test case: it checks if the function successfully retrieves a modelName
  // and returns the correct HTTP status and data.
  it('should successfully retrieve a modelName and return status 200 with the modelName data', async () => {
    // Mock data representing a modelName returned from the database.
    const mockModelNameData = {
      id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89',
      name: 'Test ModelName',
    };

    // Mock the Prisma call to find the first modelName that matches criteria.
    // It's resolved with `mockModelNameData`.
    prisma.modelName.findFirst.mockResolvedValue(mockModelNameData);
    ...
    await getModelName(req, res, next);

    // Assert the response status was called with 200.
    expect(res.status).toHaveBeenCalledWith(200);
    // Assert the response body contains the mock modelName data.
    // Controller enriches response with __displayValue and details, so use objectContaining.
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining(mockModelNameData),
    );
  ...
  // Second test case: it checks the behavior when a modelName is not found.
  // The expected behavior is to throw an error, indicating the modelName is not found.
  it('should return a 404 error if the modelName is not found', async () => {
    // Mock the Prisma call to return null, simulating not finding the modelName.
    prisma.modelName.findFirst.mockResolvedValue(null);
    // Again, mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with a non-existent modelName ID.
    ...
    await expect(getModelName(req, res, next)).rejects.toThrow();
  });
  ...
  it('throws an error if the database operation fails', async () => {
    ...
    prisma.modelName.findFirst.mockRejectedValue(new Error('Database error'));
    // Mock visibility filters with no restrictions.
    getVisibilityFilters.mockReturnValue({});

    // Mock request object with any modelName ID.
    const req = {
      params: { id: 'd8d47913-1b54-4f21-87bc-1f72d1d89f89' },
      user: {},
    };
...
describe('updateModelName', () => {
  ...
  // This test verifies that the `updateModelName` function can successfully update a modelName
  // in the database and return the updated modelName data with a 200 HTTP status code.
  it('should successfully update a modelName and return status 200 with the updated modelName data', async () => {
    // Mock data for the test, including the modelName ID and request body.
    const mockParamsId = '2454006f-fb48-4b56-b70b-48f92596fe4f';
    const mockRequestBody = {
      // @gen:MOCK_UPDATE_DATA
    };
    // The expected result after updating the modelName.
    const mockUpdatedModelName = {
      ...mockRequestBody,
      id: mockParamsId,
    };

    // Mock the Prisma updateMany operation to resolve with count indicating success.
    prisma.modelName.updateMany.mockResolvedValue({
      count: 1,
    });
    // Mock findFirst to return the updated record after updateMany.
    prisma.modelName.findFirst.mockResolvedValue(
      mockUpdatedModelName,
    );
    ...
    await updateModelName(req, res, next);

    // Assert that the response status and body are correct.
    expect(res.status).toHaveBeenCalledWith(200);
    // Verify that the Prisma updateMany method was called.
    expect(prisma.modelName.updateMany).toHaveBeenCalled();
  });

  // This test ensures that the `updateModelName` function correctly handles and throws errors
  // when the database operation fails.
  it('throws an error if the database operation fails', async () => {
    // Mock an error that would be thrown by the Prisma updateMany operation.
    const databaseError = new Error('Database error');
    prisma.modelName.updateMany.mockRejectedValue(
      databaseError,
    );
    ...
    await expect(
      updateModelName(req, {}, next),
    ).rejects.toThrow();
  });

  // This test checks that the `updateModelName` function transforms the keys of the request body
...
// This describes a test suite for the `deleteModelName` function.
describe('deleteModelName', () => {
  ...
  it('should successfully delete a modelName and return status 200 with the deleted ID', async () => {
    // Mock ID of the modelName to be deleted.
    const mockId = '5610eb16-c1dd-4be0-b940-76d1b6a8dd2c';
    // @gen:DELETE_RELATED_TABLE_MOCKS
    ...
    // Mocks the HTTP request object, including the modelName ID to delete and user authentication status.
    const req = {
      params: { id: mockId },
      user: { id: 'user-123', client: { id: 'client-123' }, isAuthenticated: true },
    };
    ...
    await deleteModelName(req, res, next);
    ...
    // containing the ID of the deleted modelName.
    expect(res.json).toHaveBeenCalledWith({ @gen{DELETE_RESPONSE_KEY}: mockId });
