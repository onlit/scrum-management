 ...
 * DESCRIPTION:
 * ------------------
 * Contract tests for ModelName API endpoints.
...
const {
  ...
  createModelName,
} = require('#tests/factories/modelName.factory.js');
const {
  ModelNameResponseSchema,
  ModelNameListResponseSchema,
} = require('#tests/core/contracts/schemas/modelName.schema.js');
const {
  NotFoundErrorSchema,
...
describe('ModelName Contract Tests', () => {
  ...
  beforeAll(async () => {
    ...
    testRecord = await createModelName();
  });
  ...
  describe('GET /api/v1/model-names', () => {
    it('should return response matching list schema', async () => {
      ...
      const { error } = ModelNameListResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('POST /api/v1/model-names', () => {
    it('should return created record matching response schema', async () => {
      // Use buildModelNameForApi to ensure required relations exist
      ...
      const { error } = ModelNameResponseSchema.validate(response.body, {
        abortEarly: false,
      });
  ...
  describe('GET /api/v1/model-names/:id', () => {
    it('should return record matching response schema', async () => {
      const response = await request(server)
        .get(`/api/v1/model-names/${testRecord.id}`)
        .set(authHeaders)
        .expect(200);
    ...
    it('should return 404 for non-existent ID with error schema', async () => {
      ...
      if (error) {
        const details = error.details.map((d) => `${d.path.join('.')}: ${d.message}`);
        throw new Error(`Error schema validation failed:\n${details.join('\n')}`);
      }
    });
  });

  describe('PUT /api/v1/model-names/:id', () => {
    it('should return updated record matching response schema', async () => {
      // Use buildModelNameForApi to ensure required relations exist
      const updateData = await buildModelNameForApi();

      const response = await request(server)
        .put(`/api/v1/model-names/${testRecord.id}`)
        .set(authHeaders)
        .send(updateData)
        .expect(200);
  ...
  describe('DELETE /api/v1/model-names/:id', () => {
    it('should return delete confirmation matching schema', async () => {
      ...
      const recordToDelete = await createModelName();

      const response = await request(server)
        .delete(`/api/v1/model-names/${recordToDelete.id}`)
        .set(authHeaders)
        .expect(200);
      ...
      if (error) {
        const details = error.details.map((d) => `${d.path.join('.')}: ${d.message}`);
        throw new Error(`Delete schema validation failed:\n${details.join('\n')}`);
      }
    });
  });
});
