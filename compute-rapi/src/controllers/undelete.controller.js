...
const { undeleteCreate } = require('#schemas/undelete.schemas.js');
...
async function undeleteBatch(req, res) {
  ...
  logOperationStart('undeleteBatch', req, { model: params?.model, user: user?.id });
  ...
  if (!models.includes(params?.model)) {
    const error = createErrorWithTrace(
      ...
      'Invalid model.',
      req,
      { context: 'undelete_batch', severity: ERROR_SEVERITY.LOW, details: { model: params?.model } }
    );
  ...
  try {
    ...
    if (error.isJoi) {
      ...
      throw createErrorWithTrace(
        ...
        req,
        { context: 'undelete_batch', severity: ERROR_SEVERITY.LOW, details: { validationErrors: error.details?.map(d => d.message) } }
      );
    }
    logOperationError('undeleteBatch', req, error);
    throw createErrorWithTrace(
      ...
      'Validation error',
      req,
      { context: 'undelete_batch', severity: ERROR_SEVERITY.HIGH, originalError: error }
    );
  }

  let result;
  try {
    logDatabaseStart('undelete_batch', req, { model: params?.model, ids: values?.ids });
    ...
    logDatabaseSuccess('undelete_batch', req, { undeleted: result?.length ?? 0 });
  } catch (error) {
    logOperationError('undeleteBatch', req, error);
    throw createErrorWithTrace(
      ERROR_TYPES.INTERNAL,
