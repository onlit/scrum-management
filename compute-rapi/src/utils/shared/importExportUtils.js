...
const { createStandardError, handleDatabaseError } = require('#utils/shared/errorHandlingUtils.js');
...
async function importBatch(recordsToCreate, modelName) {
  ...
  if (
    ...
    logEvent(`[IMPORT_EXPORT_ERROR] Invalid model name or model does not support createMany: ${modelName}`);
    throw createStandardError(
      ERROR_TYPES.BAD_REQUEST,
  ...
  try {
    ...
    count = result.count;
    logEvent(`Batch create for ${modelName} successful: ${count} records created.`);
  } catch (error) {
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      logEvent(`[IMPORT_EXPORT_ERROR] Prisma Known Error during createMany [${modelName}, Code: ${error.code}]: ${error.message}`);
    } else {
      logEvent(`[IMPORT_EXPORT_ERROR] Error during createMany [${modelName}]: ${error}`);
    }
    ...
    try {
      ...
      if (
        ...
        return {
          ...
          id,
          error: new Error(`Foreign key constraint failed: Please verify all referenced IDs exist`),
          csvRow,
          originalData: record,
        };
      }
      ...
      if (
        ...
        return {
          ...
          id,
          error: new Error(`Unique constraint violation: Record with these values already exists`),
          csvRow,
          originalData: record,
        };
      }
  ...
  for (const result of results) {
    if (result.status === 'fulfilled') {
      ...
      if (outcome.status === 'updated') {
        ...
        if (createValidationError) {
          ...
          updateErrors.push({
            ...
            errors: [
              `Update failed (ID ${id} not found), and subsequent create validation failed: ${createValidationError.details.map((d) => d.message).join(', ')}`,
            ],
        ...
        updateErrors.push({
          ...
          errors: [
            `Update failed for ID ${outcome.id}: ${outcome.error?.message || 'Foreign key constraint violation'}`,
          ],
        ...
        updateErrors.push({
          rowId: outcome.csvRow,
          errors: [
            `Update failed for ID ${outcome.id}: ${outcome.error?.message || 'Unique constraint violation'}`,
          ],
          data: outcome.originalData,
        });
        ...
        updateErrors.push({
          rowId: outcome.csvRow,
          errors: [
            `Update failed for ID ${outcome.id}: ${outcome.error?.message || 'Unknown error'}`,
          ],
          data: outcome.originalData,
        });
      }
      ...
      updateErrors.push({
        ...
        errors: [
          `An unexpected error occurred during update processing: ${result.reason?.message || 'Unknown reason'}`,
        ],
        data: {},
      });
    }
