...
const { logStep, logEvent } = require('#utils/shared/loggingUtils.js');
const { createStandardError } = require('#utils/shared/errorHandlingUtils.js');
const { ERROR_TYPES, ERROR_SEVERITY } = require('#configs/constants.js');

// Whitelist of allowed commands for security
const ALLOWED_COMMANDS = new Set(['npx', 'yarn', 'npm', 'node', 'git', 'ssh']);

// Input sanitization function
function sanitizeInput(input, checkForDangerousChars = true) {
  let sanitized = input;
    ...
    try {
      ...
      const sanitizedArgs = args.map((arg) => {
        if (typeof arg === 'string') {
          return sanitizeInput(arg, command !== 'ssh');
        }
        return arg;
      });
