 ...
 * REVISION REASON: Update error format and add error codes
 */
...
const { handleValidationError } = require('#utils/shared/errorHandlingUtils.js');
...
function errorHandler(error, req, res, next) {
  ...
  const isStandardizedError = error.type && Object.values(ERROR_TYPES).includes(error.type);
  ...
  if (isStandardizedError) {
    ...
    // Prefer explicit validationErrors on the error, then fall back to details.validationErrors
    const validationErrorsFromError = error.validationErrors;
    const validationErrorsFromDetails = error.details?.validationErrors;
    const validationErrors = validationErrorsFromError || validationErrorsFromDetails;
  ...
  if (error?.isJoi) {
    const standardizedError = handleValidationError(error, 'request_validation');
    // Extract values from standardized error and continue with normal processing
    status = STATUS_CODES[standardizedError.type] || standardizedError.statusCode || 422;
    title = 'Validation Failed';
    code = standardizedError.type;
    detail = standardizedError.message;
    validationErrors = standardizedError.details?.validationErrors || [];
  }
  ...
  if (validationErrors) {
    logEvent(
      `[TraceID: ${traceId}] [VALIDATION_ERRORS]: ${JSON.stringify(validationErrors)}`,
      traceId
    );
  }
