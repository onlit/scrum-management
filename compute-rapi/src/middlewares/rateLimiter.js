...
const generalApiLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    ...
    logEvent(`[RATE_LIMIT_EXCEEDED][GENERAL]: IP ${clientIP} exceeded general API rate limit`);
    res.status(options.statusCode).json({
      traceId: req.traceId,
      status: options.statusCode,
      title: 'Rate Limit Exceeded',
      code: 'RATE_LIMIT_GENERAL',
      detail: 'Too many requests, please try again later. Limit: 100 requests per 5 minutes.'
    });
  }
});
...
const authLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    ...
    logEvent(`[RATE_LIMIT_EXCEEDED][AUTH]: IP ${clientIP} exceeded auth rate limit. UserAgent: ${userAgent}`);
    res.status(options.statusCode).json({
      traceId: req.traceId,
      status: options.statusCode,
      title: 'Authentication Rate Limit Exceeded',
      code: 'RATE_LIMIT_AUTH',
      detail: 'Too many authentication attempts, please try again later. Limit: 10 requests per 15 minutes.'
    });
  }
});
...
const writeOperationsLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    ...
    const { method } = req;
    const { path } = req;
    logEvent(`[RATE_LIMIT_EXCEEDED][WRITE]: IP ${clientIP} exceeded write rate limit on ${method} ${path}`);
    res.status(options.statusCode).json({
      traceId: req.traceId,
      status: options.statusCode,
      title: 'Write Operations Rate Limit Exceeded',
      code: 'RATE_LIMIT_WRITE',
      detail: 'Too many write operations, please try again later. Limit: 30 requests per 5 minutes.'
    });
  }
});
...
const fileUploadLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    ...
    logEvent(`[RATE_LIMIT_EXCEEDED][FILE_UPLOAD]: IP ${clientIP} exceeded file upload rate limit. File: ${fileName}`);
    res.status(options.statusCode).json({
      traceId: req.traceId,
      status: options.statusCode,
      title: 'File Upload Rate Limit Exceeded',
      code: 'RATE_LIMIT_UPLOAD',
      detail: 'Too many file uploads, please try again later. Limit: 10 uploads per 10 minutes.'
    });
  }
});
...
const burstLimiter = rateLimit({
  ...
  handler: (req, res, next, options) => {
    const clientIP = req.ip || req.connection.remoteAddress;
    logEvent(`[RATE_LIMIT_EXCEEDED][BURST]: IP ${clientIP} exceeded burst rate limit`);
    res.status(options.statusCode).json({
      traceId: req.traceId,
      status: options.statusCode,
      title: 'Request Rate Too High',
      code: 'RATE_LIMIT_BURST',
      detail: 'Request rate too high, please slow down. Limit: 20 requests per minute.'
    });
  }
});
